...
- **Isolated-worktree exception.** `playwright-cli` (launching/attaching a real browser) is unreliable inside a sandboxed git worktree (`.claude/worktrees/...`, `.agent-worktrees/...`, or any workspace a session's sandbox reports it "cannot verify stays inside the worktree") — confirmed independently in two worktrees on 2026-09-07 (one failed fast with "could not determine executable to run", the other hung indefinitely on browser launch). The main checkout (master, no worktree isolation) does not have this restriction — P14 verification there works as written and has real precedent (the 21-screen UI redesign, 2026-08-20). **Do not burn time retrying playwright-cli inside an isolated worktree.** Instead:
