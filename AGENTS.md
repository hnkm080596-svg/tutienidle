...
**P4. Adversarial QA gate.** After a feature/fix, run `tutienidle-adversarial-qa` (quick) before claiming done — positioned after P3 verification and any triggered P13/P14 runtime checks, before the P5 review round. Deep mode: when asked, before milestone/release, or when quick QA surfaces broad risk (save/cloud, time/offline, economy/progression, Vue/Pinia/Phaser lifecycle). QA-pass writes are restricted to `**/*.test.ts`, `tests/e2e/**`, `docs/qa/**` — no production edits during QA. A defect needs deterministic repro / failing test / runtime evidence. Verdicts: `PASS WITH EVIDENCE` (QA gate passed — the P5 round still runs) / `FAIL WITH REASON` (QA satisfied only if the reason is legitimate) / `PASS WITH GAPS` / `BLOCKED` (not done). A QA-driven production fix is a normal implementation change: reverify and feed it through the P5 loop like any other fix.

**P5. Post-task gate — three-lens review round.** A non-trivial change (≥5 changed production lines, any new production file, any touched file beyond rename/comment/whitespace) is NOT complete merely because tests are green or because one review pass succeeded. Completion sequence:

`implement → simplify (E3) → verify (P3) → runtime/browser verification when triggered (P13/P14, inside the implementation worktree) → adversarial QA (P4) → three-lens review round → fix → reverify → repeat review round → done`

A **review round** = three genuinely independent passes over the same current implementation state — separate attempts to invalidate the work from materially different perspectives, not three headings in one superficial review. The `code-review` skill may feed a lens; it is not the round by itself.

- **Review A — Correctness / Regression / Requirement Fidelity.** Task requirements and acceptance criteria; changed behavior; affected callers and consumers; state/data flow; error paths; edge cases; lifecycle/reset/restore behavior where relevant; incomplete migrations; stale fallbacks; duplicate authorities; unintended regressions; whether the fix addresses the root cause rather than only the visible symptom. Actively search for evidence that the implementation is wrong.
- **Review B — Architecture / Contracts / Maintainability.** Ownership boundaries; the Architecture Constitution (A-rules); dependency direction; authoritative state/rule ownership; API and contract consistency; hidden coupling; compatibility paths; unnecessary duplication and complexity; maintainability; performance and security risks where relevant; whether a simpler coherent implementation exists. Green tests are not evidence that architecture is correct.
- **Review C — Tests / Runtime / User Flow / Adversarial Behavior.** Adequacy of unit/integration/E2E coverage; whether tests assert behavior rather than implementation details; runtime wiring; Vue/Pinia/Phaser boundaries; browser-visible behavior; real user interaction; state transitions; reset/retry/re-entry; Playwright evidence when P13/P14 triggered; missing coverage capable of hiding a real regression; failures that can occur only in the assembled application. For UI, interaction, Phaser, lifecycle, scene-transition, or browser-sensitive work this lens must use real runtime/Playwright evidence, not source inspection alone.
...
**Completion threshold.** The task is not complete while any unresolved Critical, High, or Medium finding remains within the changed or reasonably affected surface. Valid Medium-or-higher findings must be investigated, fixed, reverified, and reviewed again. Low/Nit findings need not be fixed, but every intentionally deferred one stays visible in the final report with finding, location/surface, severity, and why it is safe to defer — minor findings never silently disappear between rounds.

**Fix → re-review loop.** After each round: aggregate and deduplicate findings; fix every valid Medium-or-higher finding in scope; rerun the directly affected verification (broader verification when the fix changes shared behavior or widens the affected surface; P13/P14 runtime checks when it touches browser/runtime behavior); then run a NEW complete three-lens round — a fix invalidates previous review evidence for the surface it changes, so do not merely recheck the fixed line. Minimum: one complete three-lens round; if it ends with zero unresolved Medium-or-higher, the loop is satisfied. No maximum number of rounds. The user may explicitly waive a specific finding — record the waiver.

**P6. Multi-agent coordination.** `git status` before editing (including before delegating) — stop and notify the user if someone else's uncommitted work overlaps. Delegated agents report: worktree path, branch, files changed, verification evidence, remaining limitations. Coordinator owns aggregate diff reasoning + final verification, including the P5 review round over the aggregate diff. Use Subagent-Driven Development where Agent/Task-style dispatch exists; otherwise Inline Execution/`executing-plans` — capability decides, not habit.
...
*Playwright exposes a bug:* reproduce it in the implementation worktree, fix it in the same task worktree, rerun the failing scenario plus the relevant broader browser/E2E coverage, and feed the changed implementation through the P5 loop (a Medium-or-higher bug/fix mandates a fresh three-lens round).
...
**E3. Simplify before review.** After ~5+ changed production lines or any new production file, run `code-simplifier` before the P5 review round. Full sequence: implement → simplify → verify (P3) → triggered runtime/browser checks (P13/P14 in-worktree) → adversarial QA (P4) → three-lens review round (P5) → fix → reverify → repeat review round → done. Simplification must preserve behavior — skip and report a candidate that wouldn't. Re-simplify before the next round if a review fix adds substantial new code. User may explicitly waive.
...
**E11. Summary format.** State: what changed, relevant files/systems, behavior affected, verification evidence (P3 mode, commands, results), P13/P14 runtime/browser evidence (if triggered — from the implementation worktree, or the explicit environment blocker), QA verdict (P4, if applicable), P5 review-round result (rounds run, findings by severity, deferred Low/Nit findings with deferral reasons, any user waivers), remaining limitations, known retained debt, Notes/Suggestions.
