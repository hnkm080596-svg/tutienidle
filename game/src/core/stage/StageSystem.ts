...
export class StageSystem {
  ...
  pickNextEnemyEntry(stage: Stage, rng: () => number = () => Math.random()): StageEnemyEntry {
    return weightedRandom(stage.enemyPool.map(entry => ({ value: entry, weight: entry.weight })), rng)
  }
}
