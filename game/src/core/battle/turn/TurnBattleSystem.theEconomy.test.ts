...
import { UNG_THE_BUFF, TU_THE_BUFF, BACH_UNG_BUFF } from '../../../data/buff/TheTuBuffs'
...
// clamp through entity.maxThe ?? MAX_THE.
...
function applyBuff(participant: TurnBattleParticipant, definition: Parameters<BuffSystem['apply']>[0]): void {
...
describe('TheEconomy — pool cap + pay-per-attempt', () => {
  ...
  it('tryPayProcCost refuses below cost — no drain, no roll', () => {
    ...
    expect(tryPayProcCost(entity, resolveProcCost(new BuffPool()))).toBe(false)
  ...
  it('tryPayProcCost pays on attempt; success grants THE_PROC_GAIN back (net +5)', () => {
    ...
    expect(tryPayProcCost(entity, resolveProcCost(new BuffPool()))).toBe(true)
  ...
  it('failed proc keeps the cost down (net -15)', () => {
    ...
    tryPayProcCost(entity, resolveProcCost(new BuffPool()))
    expect(entity.currentThe).toBe(5)
  })
  ...
  it('resolveProcCost — tu_the -5; combined deltas floor at 0; bach_ung freeProcs wins', () => {
    ...
    expect(resolveProcCost(holder.buffs)).toBe(THE_PROC_COST)

    applyBuff(holder, TU_THE_BUFF)
    expect(resolveProcCost(holder.buffs)).toBe(THE_PROC_COST - 5)

    applyBuff(holder, BACH_UNG_BUFF)
    expect(resolveProcCost(holder.buffs)).toBe(0)
  })
  ...
  it('free procs still collect THE_PROC_GAIN on success', () => {
    ...
    expect(tryPayProcCost(holder.entity, resolveProcCost(holder.buffs))).toBe(true)
...
describe('the_tu_an free income (marker-gated, ordering before reactive windows)', () => {
  it('taken hit income: +6 lands on the defender BEFORE the onImpactLanded window runs', () => {
    ...
    applyBuff(playerP, UNG_THE_BUFF)
  ...
  it('round boundary grants THE_GAIN_PER_ROUND to every ung_the participant', () => {
    ...
    expect(isUngTheCombatant(playerP.buffs)).toBe(true)
    expect(isUngTheCombatant(enemyP.buffs)).toBe(false)
  })
})
