...
<style scoped>
.overlay-panel { position: absolute; inset: 0; display: grid; place-items: center; padding: 3vh 3vw; background: rgba(8, 9, 13, .76); backdrop-filter: blur(4px); }
.overlay-panel__card { max-width: 100%; max-height: 94vh; min-height: 0; display: flex; flex-direction: column; overflow: hidden; color: var(--text-primary); font-family: var(--font-body); background: linear-gradient(155deg, var(--ink-800), var(--ink-950)); border: 1px solid var(--gold-500); border-radius: var(--radius-md); box-shadow: var(--shadow-panel); }
...
.overlay-panel__heading h3 { margin: 0; color: var(--gold-500); font: 700 1.1rem var(--font-display); letter-spacing: .06em; }
.overlay-panel__close { width: 30px; height: 30px; padding: 0; color: var(--text-secondary); background: var(--ink-800); border: 1px solid var(--ink-line-soft); border-radius: var(--radius-sm); cursor: pointer; }
