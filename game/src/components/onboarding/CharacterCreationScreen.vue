...
<template>
  <main class="creation-screen">
    <header class="creation-header">
      <button type="button" @click="step === 1 ? emit('back') : step--">← Trở lại</button>
    ...
    <section v-if="step === 1" class="creation-panel name-step">
      ...
      <button class="primary" :disabled="!validName" type="button" @click="step = 2">Tiếp tục</button>
    </section>
...
<style scoped>
.creation-screen { width: 100vw; height: 100vh; box-sizing: border-box; overflow: auto; padding: 22px clamp(20px,5vw,72px) 34px; background: radial-gradient(circle at 50% -15%, #2a251b, #111116 42%, #09090c 85%); }
.creation-header { max-width: 1120px; margin: 0 auto; display: grid; grid-template-columns: 1fr auto 1fr; align-items: center; text-align: center; }.creation-header button { justify-self: start; border: 0; background: none; color: var(--text-secondary); cursor: pointer; }.creation-header span { justify-self: end; color: var(--text-muted); font-size: 12px; }.creation-header p,.kicker { margin: 0; color: var(--gold-500); font-size: 10px; letter-spacing: .25em; }.creation-header h1 { margin: 4px 0; font: 700 26px var(--font-display); }
.stepper { max-width: 420px; margin: 18px auto 24px; display: flex; align-items: center; }.stepper i { display: flex; flex: 1; align-items: center; color: var(--ink-line); }.stepper i::after { content: ''; height: 1px; flex: 1; background: currentColor; }.stepper i:last-child { flex: 0; }.stepper i:last-child::after { display: none; }.stepper b { width: 26px; height: 26px; display: grid; place-items: center; border: 1px solid currentColor; border-radius: 50%; font: 500 11px var(--font-body); }.stepper i.active { color: var(--gold-500); }
.creation-panel { max-width: 1120px; margin: auto; box-sizing: border-box; border: 1px solid rgba(255,213,79,.18); border-radius: 12px; padding: clamp(22px,3vw,36px); background: rgba(19,19,24,.94); box-shadow: var(--shadow-panel); }.creation-panel h2 { margin: 5px 0 8px; font: 600 24px var(--font-display); }.creation-panel>p:not(.kicker) { color: var(--text-secondary); }.name-step { max-width: 560px; text-align: center; }.name-step label { display: grid; gap: 8px; margin: 30px 0 8px; text-align: left; color: var(--text-secondary); font-size: 12px; }.name-step input { padding: 15px; border: 1px solid var(--ink-line); border-radius: 6px; background: #0b0b10; color: var(--text-primary); font: 600 18px var(--font-display); text-align: center; outline: none; }.name-step input:focus { border-color: var(--gold-500); }.name-step small { display: block; margin-bottom: 28px; color: var(--text-muted); }.name-step small.valid { color: var(--jade); }
.panel-heading { display: flex; justify-content: space-between; align-items: end; margin-bottom: 18px; }.panel-heading strong { color: var(--gold-300); font-size: 12px; }.talent-grid { display: grid; grid-template-columns: repeat(3,minmax(0,1fr)); gap: 10px; }.talent-card { position: relative; min-height: 128px; padding: 15px; border: 1px solid var(--ink-line); border-radius: 7px; background: #101016; color: var(--text-primary); text-align: left; cursor: pointer; transition: transform .15s,border-color .15s; }.talent-card:hover { transform: translateY(-2px); }.talent-card.selected { border-color: var(--gold-500); box-shadow: inset 0 0 0 1px var(--gold-500), 0 0 15px rgba(255,213,79,.12); }.talent-card__rarity { font-size: 10px; text-transform: uppercase; letter-spacing: .13em; }.talent-card h3 { margin: 7px 0; font: 600 15px var(--font-display); }.talent-card p { margin: 0 0 8px; color: var(--text-secondary); font-size: 11px; line-height: 1.5; }.talent-card small { color: var(--text-muted); }.talent-tier-pham .talent-card__rarity{color:var(--rank-color-1)}.talent-tier-linh .talent-card__rarity{color:var(--rank-color-3)}.talent-tier-dia .talent-card__rarity{color:var(--rank-color-5)}.talent-tier-thien .talent-card__rarity{color:var(--rank-color-7)}.talent-tier-di .talent-card__rarity{color:var(--rank-color-8)}
.panel-actions { display: flex; justify-content: space-between; gap: 12px; margin-top: 22px; }.primary,.secondary { padding: 11px 18px; border-radius: 5px; cursor: pointer; }.primary { border: 1px solid var(--gold-500); background: var(--gold-500); color: var(--gold-ink); font-weight: 700; }.primary:disabled { opacity: .35; cursor: not-allowed; }.secondary { border: 1px solid var(--ink-line); background: transparent; color: var(--text-secondary); }.attribute-step { max-width: 700px; }.points { padding: 8px 12px; border: 1px solid rgba(255,213,79,.25); border-radius: 20px; }.attribute-list { display: grid; gap: 8px; }.attribute-row { display: flex; justify-content: space-between; align-items: center; padding: 13px 16px; border: 1px solid var(--ink-line-soft); background: #101016; }.attribute-row>div:first-child { display: grid; gap: 3px; }.attribute-row small { color: var(--text-muted); }.counter { display: flex; align-items: center; gap: 16px; }.counter button { width: 28px; height: 28px; border: 1px solid var(--ink-line); border-radius: 50%; background: var(--ink-800); color: var(--gold-300); cursor: pointer; }.counter span { min-width: 18px; text-align: center; font-weight: 700; }.creation-summary { display: flex; justify-content: center; gap: 22px; margin-top: 20px; color: var(--text-secondary); font-size: 11px; }
.loading-roll { min-height: 380px; display: grid; place-items: center; color: var(--gold-300); font-family: var(--font-display); }.creation-error { margin: 14px 0 0; color: var(--crimson); text-align: center; font-size: 12px; }
