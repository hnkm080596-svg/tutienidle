...
function mountSpringPanel() {
  ...
  const app = createApp({ render: () => h(LeftPanel) })
...
describe('LeftPanel — building header và Linh Tuyền', () => {
  it('render Linh Tuyền trong LeftPanel theo thứ tự Tên → Cấp → Nâng cấp', async () => {
    ...
    expect(firstTags).toEqual(['H2', 'P', 'BUTTON'])
