  ...
  try {
    ...
    if (!readMode) {
      ...
      if (isTextarea) {
        ...
        // ProseMirror: focus, clear any leftover draft, then insert. Small
        // prompts use keyboard.insertText (real input events). Large prompts
        // go through a synthetic paste event instead: insertText serializes
        // the editor on every input and a ~70KB diff pegs the renderer for
        // minutes, while doPaste parses the payload once.
        await compEl.click()
