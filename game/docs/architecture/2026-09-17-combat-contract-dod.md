...
| 12 | Reaction snapshot records exact participating instances/stacks | Mechanism DONE; production sibling | `CombatOperationBatchRunner.test.ts` "buff_participant requires an exact source/target/stacks match" — the precondition primitive consumes exact instance/source/target/stacks. Snapshot production: reaction megaplan. |
...
| 25 | Same initial state + same RNG seed produces identical final trace and combat state (sec.87) | DONE | `TurnBattleSystem.rngContract.test.ts` "two battles on the same SeededCombatRng produce identical outcomes" (+ different-seed divergence guard); `rng.test.ts` SeededCombatRng sequence identity + rollChance one-consumption parity. |
