...
For work admitted under the internal QA run (schema v2), the same intake also produces a **construction brief** — `qa:internal prepare` drafts it from the task card, `qa:internal preflight` checks it. It extends the card with: bound authority/dependency refs with content hashes (`sourceDependencyHashes`), the deterministic lesson routing record (`lessonRouting`), ordered `slices`, `firstProofObligations` (oracle + stage BEFORE_WRITE/BEFORE_PROPAGATION + admittedSlice), and the readiness verdict. Readiness outcomes: `IMPLEMENTATION_READY` (all obligations resolvable now), `DISCOVERY_REQUIRED` (a binding question must be answered before write), `SCOPE_DECISION_REQUIRED` (human decision needed), or `STALE`/`DRAFT` (not admissible). A run with `requiredReadiness` (default for new production runs) cannot claim fixed-point completion while a declared brief is unmet (protocol clause C9).

Each expected file needs a reason such as owns rule, calls owner, shares contract, persists affected state, tests invariant or documents changed contract. `firstProof` picks the highest-cost uncertain assumption and proves it first — BEFORE_WRITE before the first production line, BEFORE_PROPAGATION before the next slice ships. When the brief's planningBaseline drifts (source or contract changed under the task), record a `qa:internal checkpoint`; affected slices are revalidated before continuing. A file list is a forecast, not a reason to patch the wrong layer. A required additional consumer in the same responsibility can be added with evidence. A separate feature, ownership migration or product decision requires existing authorization or a user decision.
