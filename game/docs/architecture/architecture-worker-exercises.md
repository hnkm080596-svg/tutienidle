...
| T13 | Reuse canonical tooltip and measured shared projection; real browser resize/drag/drop and inspect visual alignment under P14 inside the implementation worktree — a genuine environment failure is an explicit blocker with evidence, not a deferral. | Copied dimensions accepted from a single screenshot or silent P14 omission. |
