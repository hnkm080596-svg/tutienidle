...
- **Không mô phỏng:** không `TurnBattleSystem`, không animation. Mỗi cycle hoàn thành → `rollAutoFarmCycleReward`: dựng danh sách "dead enemies" (`RewardPendingEnemy[]`) từ `stage.enemyPool` (số lượng = `effectiveTotalEnemyCount(stage)`, spawn qua `enemySystem` + `pickEnemyForTurnSpawn` cho final/boss), rồi tái dùng `BattleLootSystem.processDefeatedEnemies` — bounty/talent/drop giống hệt trận thật, channel `'idle'`; heal-on-kill truyền `healTarget = null` (opt-out có chủ đích — kênh idle không có player entity đang giao chiến, không dựng Battle giả, F3 2026-09-13). `try/finally` restore channel `'active'` để exception không rò state sang trận kế.
