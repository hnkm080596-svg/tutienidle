...
| K10 | **37 authored combos, prefix-free, CI-validated.** 15 at length 3, 12 at length 4, 10 at length 5 (pattern table §4.3). No combo is a prefix of another; an architecture test rejects any new entry that violates it. Each combo has its own authored effect — not a per-length formula. |
| K11 | **Discovery is hardcore — no hints.** No cast-log HUD, no codex, no silhouette list, no "combo fired" banner. A combo's only tell is its effect/VFX/damage payload. 3875 possible patterns, 37 real — 98.5% dead by design. |
...
| K13 | **Ngu kit: 1 active + 2 passive emblems.** Basic `ngu_kiem_thuat` (active): summons `kiemDaoCount` phi kiếm, all striking the primary target — each an independent damage instance dealing `kiemDaoBase × might` physical, each rolling the Roll Cascade, each independently proccing on-hit/thorns/leech (thorns ×N is an accepted, flagged risk). Surplus swords on a mid-cast kill are wasted — no retarget. +1 Kiếm Ý per cast. Special slot = **Tụ Kiếm Ý** passive emblem showing `X / 9999` progress (not castable). Ultimate slot = **Kiếm Đạo Roll Cascade** passive emblem (not castable) — same emblem pattern as Phap An's `ngo_dao_hon_don`. |
| K14 | **Kiếm Ý → Kiếm Đạo economy.** Kiếm Ý is a banked, persisted pool (no in-battle reset). Sources: +1 per `ngu_kiem_thuat` cast and Cửu Cung node grants (bought with Cảm Ngộ). Auto-conversion: every 9999 Kiếm Ý → +1 Kiếm Đạo (one more phi kiếm), while `kiemDaoCount < cap`. Cap = `realmIndex + 1` (Luyện Khí 2 … Độ Kiếp 10 — see §11 for the 10-vs-11 note). Excess Ý stays banked for the next realm — never wasted. |
| K15 | **Hợp nhất on breakthrough: count resets to 1, base grows.** On realm advance, the Ngu player's phi kiếm fuse: `kiemDaoBase ×= (1 + MERGE_BONUS × kiemDaoCount)` (MERGE_BONUS = 0.1 tunable — 7 swords → ×1.7 base), then `kiemDaoCount = 1`. The temporary damage dip IS the design — each realm is a forge loop (1→cap→merge), and "đột phá sớm hay cày thêm" is the path's core tension. `kiemDaoBase` persists and compounds across realms. |
...
| K17 | **Execute is damage, not a kill flag.** `a` = a huge damage multiplier on that instance when `target.hp% < x%` — resolved through the normal pipeline (SurviveLethalGuard, ward, block all still apply). Boss-safe by construction: it is a burst, not an instant-kill. `x% = 10% × realmIndex` (caster), cap 50% (Trúc Cơ unlock → 20%). |
| K18 | **Ngu never misses; baseline never executes.** Locked `a` → always `b` (no execute). Roll 2 fail → `f` guaranteed hit (bypass evasion); since `e` adds a crit roll on top, Ngu's hit ALWAYS lands — the accuracy/evasion axis is dead for this path (deliberate identity: Ngự Kiếm bách phát bách trung). Locked `d` → no pierce roll → normal armor-mitigated damage. |
...
### 4.3 Combo table (37 — prefix-free, locked pattern set)
...
B-H-C-B Phách Liêu Trảm Phách      Đ-C-C-Đ Thích Trảm Trảm Thích
C-Đ-Đ-C Trảm Thích Thích Trảm      B-Đ-Đ-B Phách Thích Thích Phách
```
...
| special | `tu_kiem_y` | passive emblem — Kiếm Ý progress `X / 9999` (never castable; `selectAction`/`selectForcedAction` treat it as absent) |
...
- Each phi kiếm: `damage = kiemDaoBase × might`, physical, primary
  ...
  on-hit/thorns/leech. No retarget; overkill is lost.
...
for each phi kiếm instance:
  Roll 1 (Trảm Sát)   [node 'a' unlocked?]:
      target.hp% < x%  → a: instance gains EXECUTE_MULT damage
      else             → b: nothing
  Roll 2 (Chí Mạng)   [node 'e' unlocked?]:
      roll success     → e: this instance is a guaranteed crit
      else             → f: guaranteed hit (bypass evasion)
  ...
  → instance enters the normal damage pipeline
    (accuracy — already guaranteed by f; crit — e forces it,
     pipeline crit still rolls on non-e instances; armor — c skips
     it, d reduces it, baseline applies it)
```

- Locked state → the roll DOES NOT happen and the default applies:
  no `a` → always `b`; no `e` → always `f` (still guaranteed hit);
  no `d` → normal armor mitigation (neither `c` nor `d`).
...
```text
gain:   +1 kiemY per ngu_kiem_thuat cast; +<node-granted> per Cửu Cung node
convert: while (kiemY >= 9999 AND kiemDaoCount < realmCap):
             kiemY -= 9999; kiemDaoCount += 1
cap:    kiemDaoCap = realmIndex + 1        // LK 2, TC 3, KĐ 4, NA 5,
                                          // HT 6, LH 7, HThể 8, ĐT 9, ĐK 10
merge (on realm breakthrough):
    kiemDaoBase *= (1 + 0.1 * kiemDaoCount)
    kiemDaoCount = 1
```
...
- The merge dip is intentional (K15): pre-breakthrough count is
  banked power, not current power.
...
4. **Prefix-free data:** the combo table contains no pattern that is a
   prefix of another (architecture test).
...
9. **Cascade bounds:** rolls happen only for unlocked states; `f`
   means Ngu instances never miss; `a` is a damage multiplier — it can
   never bypass SurviveLethalGuard/ward semantics.
10. **Kiếm Ý economy:** only casts (+1) and Cửu Cung grants add Ý;
    conversion is all-or-nothing 9999→1, automatic, blocked at cap;
    merge happens exactly once per breakthrough and always resets
    count to 1.
...
- **37 combo effects are the largest authored-content task** — this
  spec fixes the contract (§4.2) and the pattern table; each combo's
  effect is authored in the implementation plan's content pass. Rule
  of thumb: payoff scales with pattern length and obscurity; the two
  Ngũ Hành len-5 combos are the jackpot tier.
- **Độ Kiếp cap is 10 by formula (`realmIndex + 1`)** — the user
  ballparked 11; if 11 is wanted the formula becomes `+2`. Flagged,
  not blocking — balance-pass tunable either way.
...
- **UI debt acknowledged:** hien preset editor (9-slot strip + orb
  ...
  Ngu emblems (Kiếm Ý `X/9999` progress + cascade status), the
  `kiem_tu_an` hidden-node reveal with its permanent/no-refund
  warning, breakthrough merge feedback ("9 phi kiếm hợp nhất — base
