...
> **Revision (2026-09-18 — pre-implementation rulings locked):** spec unparked → FINAL/READY v1.1 (§24 tie-break = authored `selectionTiePriority`; `reactionEligible: boolean` → `reactionEligibility: 'eligible'|'suppressed'` — both Contract v1.5 supremacy). Ruling table below is now LOCKED: R3/R7/R-B confirmed, R-A/R-D/R-E locked, R-E2 locked as transitional (retires when SkillDefinition migration reaches full `actionTags` coverage), R-C DEFERRED — `potency` is NOT the penetration channel; the semantic is undecided until the seal batch. "Inert" scope clarified: reaction evaluation/dispatcher stays unwired, but `ActionValidator`/`forbiddenActionTags` (M4) is production-capable restriction infrastructure landing in the live turn engine — intended, not a contradiction.
>
...
**Goal:** Build the capability-gated Ngũ Hành reaction engine for Ngộ Đạo (hidden Pháp Tu): same-source/same-target reaction board, Sinh `P²` / Khắc `A×D` deterministic candidate selection, consume-all semantics, snapshot+precondition atomicity, and pure `ResolvedCombatOperation` emission — shipped with reaction evaluation **unwired** (no dispatcher, nothing in production grants `elemental_reaction_enabled`) and **fully test-covered** on fixture element/ailment ids. `ActionValidator`/`forbiddenActionTags` is the one exception: it is production-capable restriction infrastructure landing in the live turn engine at M4 (see Global Constraints).
...
- **Reaction evaluation ships UNWIRED — through M-INT (R7 — LOCKED 2026-09-18; r4+r5-amended):** `elemental_reaction_enabled` is granted by nothing in production during M1–M6, AND M-INT grants it to nobody either — the visible-Pháp Tu seam is NOT a grant site (spec: only Ngộ Đạo auto-reacts). No `TurnBattleSystem`/`GameManager*` wiring during M1–M6; M-INT does NOT register the dispatcher (r5 — no valid production `ReactionRegistry` exists yet). The engine becomes live only when the canonical ấn defs AND the Ngộ Đạo capability grant land (hidden-path content — outside this program). All coverage is Vitest-level on fixture content. **Scope of "unwired" (2026-09-18 clarification):** it covers reaction evaluation/dispatch only. `ActionValidator`/`forbiddenActionTags` (M4) is generic action-restriction infrastructure and DOES land in the live turn-engine selection path (`TurnSkillAction`/`TurnBattleSystem`) — production-capable by design; the P3-full + P13/P14 gates at M4 exist precisely because of it.
- **Legacy reaction path untouched UNTIL M-INT (R3 — LOCKED 2026-09-18):** `TurnReactionManager` (`src/core/battle/turn/TurnReactionManager.ts`, rules engine over `WuxingRelations`) keeps serving visible Pháp Tu via `canInitiateWuxingReactions` (`TurnBattleSystem.ts:137`, stamped at `TurnBattleAdapter.ts:60`) through this program's M1–M6. Its removal is NOT seal-batch scope — it is owned by **M-INT** inside the buff megaplan's M4 worktree (the buff cutover deletes the `BuffPool` the legacy engine reads and mutates, so the swap must land on that same branch before merge — buff-plan r4 BLOCKER 3). **r4 amendment:** the removal is NOT paired with a replacement grant for visible Pháp Tu — per the Final Spec, visible Pháp Tu has no automatic Ngũ Hành reactions; the legacy auto-reaction behavior is spec-nonconforming and is retired, not preserved.
