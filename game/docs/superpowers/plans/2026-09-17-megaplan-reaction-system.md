...
> **Revision (buff-plan review round 4, `b8bd751b`):** M-INT's shape is corrected per the Final Spec — (a) `elemental_reaction_enabled` is NOT granted at the `canInitiateWuxingReactions` seam: visible Pháp Tu never auto-reacts (the spec grants automatic reactions to Ngộ Đạo/hidden Pháp Tu only; the grant lands with hidden-path content, seal-batch scope); (b) `ElementalStateRegistry` binds the five canonical ấn ids (`hoa_an`/`han_tuc`/`doc_can`/`liet_thuong`/`tran_an`) — legacy ailments (`te_cong`/`chay_mau`/`thach_hoa`…) are NEVER promoted into the canonical board (spec: they are not renames). M-INT therefore wires the dispatcher and deletes the legacy engine, leaving the new engine registered but provably inert — `not_elemental` on every legacy application, `capability_missing` on every source.
>
> **Revision (buff-plan review round 3, `38e90b3e`):** the rollout conflict with the Buff megaplan is resolved by a single integration sequence — new milestone **M-INT** (production integration) is scheduled INSIDE the buff megaplan's M4 worktree, after its consumer cutover removes the legacy `checkAndTrigger` call site. M-INT owns: registering `ReactionDispatcher` on `elemental_application_committed` and deleting `TurnReactionManager` + the flag (r4-amended: NO visible-path capability grant, NO legacy-ailment rebinding). "Inert until seal batch" is amended to "registered but inert until canonical content lands" everywhere below.
...
**Exit criteria:** spec §74–82 + contract §91–101 reaction-relevant rows all named and green; sequential settlement proven; trace reconstructable; docs updated; engine remains production-inert (M-INT registers the dispatcher but grants nothing — inert until canonical content lands).
...
- [ ] `ElementalStateRegistry` production binding → the five canonical ấn ids (`hoa_an`/`han_tuc`/`doc_can`/`liet_thuong`/`tran_an`). **Never** migrated legacy ailments — `te_cong`/`chay_mau`/`thach_hoa`/etc. are spec-distinct states and must not masquerade as canonical. Effect: every legacy elemental application gates `not_elemental`.
- [ ] `CANONICAL_REACTIONS` stays `test_*`-bound for unit coverage and is NOT loaded into the production `ReactionRegistry` until the real payoff/state def ids exist (its `buffExists` validation would fail on fixture ids; an unloaded registry is legal — the element gate rejects everything before selection is reachable).
...
- [ ] Verify: buff M4's full gate owns the evidence — the Playwright battle check asserts a visible-Pháp-Tu elemental application fires NO automatic reaction (spec-correct), and unit tests prove the gate verdicts (`not_elemental` for legacy ailments, `capability_missing` for all sources).
...
4. RESOLVED (rollout lock, r4-amended): `CANONICAL_REACTIONS` ships in `src/data/reaction/` bound to `test_*` fixtures; production loading waits for the real payoff/state def ids (seal batch) — M-INT binds `ElementalStateRegistry` to the five canonical ấn ids and does NOT rebind legacy ailments into the canonical board.
