...
- **Mission A dependency:** Mission A's plan adds `assignedWorkers` persistence + an `activeCycle` element check in `validateProductionSitesSave`. This plan's Task 3 deletes that check; if executed before A lands, the equivalent edit applies to whatever productionSites validation exists then.
