...
export type AuthoredSkillOperation =
  ...
  | { type: 'heal'; target: SkillTargetIntent; amount?: number; fractionOfMaxHp?: number; fractionOfPriorDamage?: { fraction: number; capFractionOfHealTargetMaxHp: number } }   // authored-level result reference → executor emits a DeferredOperation (pre-minted `operationId`, cap resolved by HealAuthority — contract v4 R-C7)
