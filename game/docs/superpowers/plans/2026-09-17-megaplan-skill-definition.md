...
| R-S4 | `appliesAilment(s)` → authored `apply_ailment` op carrying `reactionEligibility:'eligible'` for normal elemental applications — eligibility is producer-path metadata, NOT derived from `actor.canInitiateWuxingReactions` (r4-review MEDIUM 1: `'suppressed'` is for recursion-suppressed lanes only; whether a reaction runs is the ReactionSystem's capability+registry gate). The legacy `TurnReactionManager` is deleted at reaction **M-INT** inside the buff M4 worktree (reaction plan R3 — not seal batch), with NO replacement grant to visible `phap_tu`. | Contract §14–16: eligibility is request metadata; who decides = caster authority at resolve time. |
