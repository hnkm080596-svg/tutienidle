...
> **Review revision:** v5 — incorporates code review round 4 (verdict REQUEST CHANGES on `b8bd751b`: 3 BLOCKER / 4 HIGH / 4 MEDIUM). Fixes in this revision:
...
> - **BLOCKER 2 (reaction):** M-INT no longer grants `elemental_reaction_enabled` at the visible-Pháp Tu seam — per the Reaction Final Spec, visible Pháp Tu never auto-reacts; the grant is hidden-path/Ngộ Đạo content scope. Post-M-INT the dispatcher is registered but provably inert.
...
**Specs:**
- `game/docs/specs/2026-09-17-buff-system-reimagined-spec.md` (v1.3 — THE source of truth; section numbers below cite it; v1.1–v1.3 addenda lock the implementation deltas)
- `game/docs/specs/2026-09-17-combat-systems-contract-spec.md` (v1.4 — `ApplyBuffRequest`/`ApplyBuffResult`, removal reasons, event payloads, batch semantics, `PeriodicOperationSettled` + typed `periodicRequestId`; supersedes conflicting points in the buff spec)
...
9. **v7.3/v7.4 — `PeriodicOperationSettled` event** (BLOCKER 1, contract spec v1.3/v1.4): scheduler-originated (NOT in `CombatEventPayload`/`PendingCombatEvent`); emitted into the enclosing frame after every periodic op's barrier — detected via the op's typed `periodicRequestId` field (v7.4 — never id parsing), stamped `evt.settled.${operationId}` / `causationOperationId` / `rootActionId` / `allocateSeq()`. BuffSystem's registered handler finalizes `uses` pending-marks (by `modifierRuntimeId`) AND drives manual-trigger continuation through the event-scoped sink. `settle()` may be called once per sequential periodic unit (BLOCKER 2).
...
export class BuffSystem {
  ...
  // Manual periodic trigger (spec §27/§62 + r4-review BLOCKER 1): emits only
  // the FIRST unit's single-request PeriodicRequestsCommitted via ctx.events
  // and stores the remaining unit list as a pending continuation keyed by the
  // emitted requestId — subsequent units are emitted one-at-a-time by
  // handlePeriodicSettled (below), each computed against post-settlement
  // state. NEVER advances lifetimes, NEVER decrements turn-based modifier
  // lifetimes. 'uses' marks stay PENDING until the generated op's
  // PeriodicOperationSettled ('resolved' → consume, else release).
...
Manual triggerPeriodic (r4-review BLOCKER 1 — settled-event CONTINUATION):
  ...
  return — NO internal settle; the calling op's barrier drains the request
    event → generated periodic op → PeriodicOperationSettled →
    handlePeriodicSettled finalizes marks THEN emits the next unit's request
    through the event-scoped sink → its op settles → its settled event drives
    the following unit — all INSIDE the triggering op's barrier.
...
| `reactionManager.checkAndTrigger(...)` `:2979` | **REMOVED (spec §73 — no dual-run) — by reaction megaplan M-INT, scheduled inside THIS worktree (r4 BLOCKER 3 — single integration sequence).** Order inside M4: (a) data migration + consumer cutover (this step deletes the call site as part of the `applySkillAilments` rewrite — it reads AND mutates `BuffPool`, so it cannot survive the pool removal); (b) **M-INT lands on the same branch — corrected shape (r4-review BLOCKERs 2+3):** `ElementalStateRegistry` binds elements → the five canonical ấn ids (`hoa_an`/`han_tuc`/`doc_can`/`liet_thuong`/`tran_an`) — NEVER migrated legacy ailments (`te_cong`≠`han_tuc`, `chay_mau`≠`liet_thuong`, `thach_hoa`≠`tran_an`); `ReactionDispatcher` registers on `elemental_application_committed`; `TurnReactionManager` + `canInitiateWuxingReactions` delete; `elemental_reaction_enabled` is NOT granted to visible `phap_tu` (spec: only Ngộ Đạo auto-reacts — the grant is hidden-path content scope). Post-M-INT the engine is wired but inert: every legacy application gates `not_elemental`, every source gates `capability_missing`. **Merge gate:** M4 is not merge-ready until M-INT completes — the legacy engine/flag must be fully deleted, not left dangling (P14 verifies visible Pháp Tu does NOT auto-react). |
...
- [ ] **Step 4 — Land reaction megaplan M-INT on this worktree** (r4 BLOCKER 3 — the single integration sequence; corrected per r4-review BLOCKERs 2+3): `scheduler.registerImmediateHandler('elemental_application_committed', ReactionDispatcher)`; `ElementalStateRegistry` binds the five canonical ấn ids (never legacy ailments); `CANONICAL_REACTIONS` stays `test_*`-bound for unit tests and is NOT production-loaded until the real payoff/state def ids exist (the element gate rejects every legacy application before selection is reachable); `elemental_reaction_enabled` granted to NOBODY — the Ngộ Đạo grant is hidden-path content scope; delete `TurnReactionManager`/`canInitiateWuxingReactions`. **Merge gate — M4 does not merge until this step is green.**
- [ ] **Step 5 — Verify (P3 FULL)** + **P13/P14 Playwright** real battle from the worktree: ailment applies, DoT ticks (damage lands before expiry), buffs expire, CC blocks a turn, and — post-M-INT — a visible-Pháp-Tu elemental application fires NO automatic reaction (spec-correct: the dispatcher gates `not_elemental`/`capability_missing`; the engine activates only when canonical defs + the Ngộ Đạo grant land). **P4 deep + P5 round.**

**Exit criteria:** no production `new BuffPool()` remains except inside `BuffPersistence`; all data migrated to the new shape (zero legacy-shape defs); no `TurnReactionManager` invocation on the ailment lane AND `TurnReactionManager`/`canInitiateWuxingReactions` deleted (M-INT); `elemental_application_committed` → `ReactionDispatcher` wired and PROVABLY INERT (visible Pháp Tu no longer auto-reacts — spec-correct; canonical-state defs + the Ngộ Đạo capability grant are seal/hidden-path scope); full suite green; real battle verified.
...
| §34 + v7.3/v7.4 | `uses` consumed iff generated op `'resolved'` — finalized by `PeriodicOperationSettled` inside the same tree; manual trigger resolves → consumed, manual trigger skipped → survives; **manual multi-unit continuation:** A skipped → B folds the released modifier, A resolved → B does not, A's consequence mutates state → B sees it; skipped crossings release marks so a LATER crossing folds the modifier; **exact-entry identity:** same `modifierId` diff `appliedBy` / `reapply:'stack'` / mid-settlement replacement → only the marked `modifierRuntimeId` generation finalizes; **`uses:1`→0 removal emits `buff_modifier_removed`** | `BuffPeriodic.test.ts :: manual uses finalize` / `:: manual skip retains` / `:: manual continuation sequential` / `:: pending mark entry identity` / `:: uses removal is evented` / `BuffLifecycle.test.ts :: interval skip releases uses` |
...
| r5 B2/B3 (M-INT) | `ElementalStateRegistry` binds only the five canonical ấn ids — legacy `te_cong`/`chay_mau`/`thach_hoa` applications gate `not_elemental`, never masquerade; no `elemental_reaction_enabled` grant on the visible-`phap_tu` seam → `capability_missing` — visible Pháp Tu never auto-reacts post-cutover | `ReactionGate.production.test.ts :: legacy ailments not canonical` / `:: visible phap tu no reaction` |
...
6. RESOLVED (review rounds 3+4 — r4 BLOCKER 3 + r5 BLOCKERs 2–3): the rollout conflict is locked to ONE sequence — reaction megaplan **M-INT** runs inside this plan's M4 worktree (after the consumer cutover that deletes the `checkAndTrigger` call site): registers `ReactionDispatcher`, binds `ElementalStateRegistry` to the five canonical ấn ids, deletes `TurnReactionManager` + the flag — and grants `elemental_reaction_enabled` to NOBODY (spec: Ngộ Đạo-only; visible-Pháp-Tu auto-reactions are retired, not preserved). No dangling legacy path exists — both sides of the swap land before merge.
