...
> **Review revision:** v7 — incorporates code review round 6 (verdict REQUEST CHANGES on `3007986f`: 0 BLOCKER / 1 HIGH / 3 MEDIUM / 2 MINOR). Fixes in this revision:
> - **HIGH 1 (buff spec v1.5):** spec §22–23 canonicalized — `PeriodicDamageDefinition` gains `snapshotFields?`/`canMiss`/`hitCount`/`tags?`; `PeriodicDamageRequest` replaced by contract's `BuffPeriodicDamageRequest` (with `requestId`/`instanceId`/`canMiss`/`hitCount`/`tags?`/`snapshot?`); the request carries NO `origin`/`originId` — the scheduler bridge owns operation provenance (`kind:'buff_periodic'`).
> - **MEDIUM 1:** manual trigger gains an explicit all-dead branch — matched-but-dead candidates → `{started:false, candidateUnitCount:<list size>}`, no request, no pending series.
> - **MEDIUM 2:** spec header's stale "gap cần ruling" notes replaced by the RESOLVED rulings (R1/R2/migration).
> - **MEDIUM 3:** skill plan's dependency refs resynced (contract v1.5, buff v1.5).
> - **MINOR 1–2 (recorded):** future exact-entry modifier removal = separate `RemoveBuffModifierEntryOperation{modifierRuntimeId}` (never overload `remove_buff_modifier`); scheduler-private correlation map records after group reservation / clears on structural fault.
>
> v6 — incorporates code review round 5 (verdict REQUEST CHANGES on `cb3a602f`: 3 BLOCKER / 3 HIGH / 3 MEDIUM). Fixes in that revision:
> - **BLOCKER 1 (contract v7.5):** `triggerPeriodic` returns `TriggerPeriodicStartResult {started, firstRequestId?, candidateUnitCount}` — start metadata only; the op can no longer claim resolutions for continuation units that don't exist yet at return time. Per-request outcomes stay observable via `PeriodicRequestsCommitted`/`PeriodicOperationSettled`/trace.
> - **BLOCKER 2 (reaction):** M-INT SIMPLIFIED — it removes the legacy engine/flag and keeps the canonical `ElementalStateRegistry`, but does NOT register `ReactionDispatcher` (no valid production `ReactionRegistry` can be constructed while `CANONICAL_REACTIONS` stays fixture-bound). Registration defers to the seal/Ngộ Đạo mission where real canonical content exists.
> - **BLOCKER 3 (contract):** contract spec v1.5 addendum supersedes §4 — `ResolvedCombatOperation` has NO top-level `sourceId`; `origin.sourceId` is the sole runtime source authority.
> - **HIGH 1:** `modifierRuntimeId` propagated to the public surface — `buff_modifier_added`/`buff_modifier_removed` events carry it, `addModifier` returns it, `removeModifier` is locked `all_matching` semantics (removes every entry with the authored id, one removal event per generation).
> - **HIGH 2 (contract v7.5):** `PeriodicOperationSettled.eventId` mints through the canonical per-scope allocator (`evt.${opId}.${scopeOrdinal++}` — the op's own scope shares the sink's counter) — supersedes v5's `evt.settled.${opId}`, which was collision-proof only among settled events.
> - **HIGH 3 (contract v7.5):** periodic correlation is scheduler-PRIVATE — `ResolvedCombatOperation.periodicRequestId` deleted; the built-in bridge records `operationId → requestId` in a scheduler-internal map. Nothing public to forge.
> - **MEDIUM 1–3:** spec headers resynced (buff spec v1.4, contract spec v1.5 — the addenda now match the advertised versions); reaction-plan dependency refs updated (v1.3→v1.5, v1.2→v1.4); M-INT test wording corrected — legacy ailments never reach the dispatcher (no `ElementalApplicationCommitted` is emitted), so the inertness proof is "dispatcher not called," not a fabricated gate verdict.
>
> v5 — incorporates code review round 4 (verdict REQUEST CHANGES on `b8bd751b`: 3 BLOCKER / 4 HIGH / 4 MEDIUM). Fixes in that revision:
> - **BLOCKER 1:** manual `triggerPeriodic` is now truly sequential — it emits only the FIRST unit's single-request `PeriodicRequestsCommitted` and stores the rest as a pending continuation; `handlePeriodicSettled` (now receiving the event-scoped sink) finalizes marks, then computes + emits the NEXT unit's request through that sink. `compute → settle → finalize → next` holds identically on manual, lifecycle, and interval paths.
> - **BLOCKER 2 (reaction):** M-INT no longer grants `elemental_reaction_enabled` at the visible-Pháp Tu seam — per the Reaction Final Spec, visible Pháp Tu never auto-reacts; the grant is hidden-path/Ngộ Đạo content scope. (v6 superseded the "registered but inert" half — registration defers to the canonical-content mission; see above.)
> - **BLOCKER 3 (reaction):** M-INT no longer rebinds `ElementalStateRegistry`/`CANONICAL_REACTIONS` to migrated legacy ailments — the registry binds the five canonical ấn ids only; `te_cong`/`chay_mau`/`thach_hoa` never masquerade as `han_tuc`/`liet_thuong`/`tran_an`.
> - **HIGH 1:** pending `uses` marks identify the runtime modifier ENTRY (`modifierRuntimeId` — per-instance monotonic; replace/max/min overwrites mint a new one), so a same-id replacement inside a consequence tree is never consumed by an earlier reservation.
> - **HIGH 2:** `handlePeriodicSettled(event, sink)` — finalization-removal emits `buff_modifier_removed` through the sink.
> - **HIGH 3 (contract v7.4):** periodic correlation is typed — `ResolvedCombatOperation.periodicRequestId`; the scheduler never parses `operationId` strings.
> - **HIGH 4 (contract v7.4):** `PeriodicOperationSettled` stamping locked — `eventId='evt.settled.'+operationId`, `causationOperationId=operationId`, `rootActionId` copied from origin, seq via `allocateSeq()` (→ `seq(op) < seq(settled)`), recorded exactly once.
> - **MEDIUM 1–4:** `reactionEligibility` decoupled from the legacy path flag (normal apply → `'eligible'`; only recursion-suppressed lanes → `'suppressed'`); spec references resynced (buff spec → v1.3, contract spec → v1.4 — new addenda lock this round's deltas); contract spec `Applies to` fixed (Buff v1.2); reaction-plan M5 stale seal-batch wiring wording retargeted to M-INT.
>
> v4 — incorporates code review round 3 (verdict REQUEST CHANGES on `38e90b3e`: 3 BLOCKER / 5 HIGH / 4 MEDIUM). Fixes in that revision:
...
> - **BLOCKER 3:** rollout conflict resolved — the reaction megaplan gains an explicit **M-INT** integration milestone (register `ReactionDispatcher` on `elemental_application_committed`, delete `TurnReactionManager` + the flag) scheduled inside this plan's M4 worktree; M4's merge gate now references M-INT instead of contradicting the reaction plan's inert-until-integrated ruling. (v4 described a capability grant + legacy-ailment rebinding at that seam — superseded in v5: no grant to visible `phap_tu`, no legacy-into-canonical rebinding.)
...
**Specs:**
- `game/docs/specs/2026-09-17-buff-system-reimagined-spec.md` (v1.5 — THE source of truth; section numbers below cite it; v1.1–v1.5 addenda lock the implementation deltas)
- `game/docs/specs/2026-09-17-combat-systems-contract-spec.md` (v1.5 — `ApplyBuffRequest`/`ApplyBuffResult`, removal reasons, event payloads, batch semantics, `PeriodicOperationSettled` + scheduler-private periodic correlation; supersedes conflicting points in the buff spec)
...
9. **v7.3–v7.5 — `PeriodicOperationSettled` event** (BLOCKER 1, contract spec v1.3–v1.5): scheduler-originated (NOT in `CombatEventPayload`/`PendingCombatEvent`); emitted into the enclosing frame after every periodic op's barrier — detected via the scheduler-PRIVATE `operationId → requestId` map the built-in bridge records (v7.5 — no public field, never id parsing); `eventId` mints through the canonical per-scope allocator `evt.${opId}.${scopeOrdinal++}` (v7.5), `causationOperationId`/`rootActionId`/`allocateSeq()` as locked in v7.4. BuffSystem's registered handler finalizes `uses` pending-marks (by `modifierRuntimeId`) AND drives manual-trigger continuation through the event-scoped sink. `settle()` may be called once per sequential periodic unit (BLOCKER 2).
10. **v7.5 — `triggerPeriodic` → `TriggerPeriodicStartResult`** (r5 BLOCKER 1) + **modifier runtime identity on the public surface** (r5 HIGH 1): `buff_modifier_added`/`removed` carry `modifierRuntimeId`; `addModifier` returns it; `removeModifier` removes every entry of the authored id (`all_matching`).
...
// Runtime entry identity (r4-review HIGH 1): each inserted entry ALSO gets a
//   `modifierRuntimeId` = `mod.${instanceId}.${entryOrdinal}` — per-instance
//   monotonic, minted at insert. 'replace'/'max'/'min' overwrites mint a NEW
//   runtime id (new generation); 'stack' appends always mint one. Pending
//   uses-marks and `removeModifier` target this id — a same-`modifierId`
//   replacement inside a consequence tree is a different generation and can
//   never be consumed by an earlier request's reservation (its mark dies with
//   the replaced entry).
...
export class BuffSystem {
  ...
  // v6 — runtime-entry identity on the public surface (r5 HIGH 1):
  // addModifier reports the minted modifierRuntimeId; removeModifier is
  // all_matching — removes EVERY runtime entry carrying the authored
  // modifierId (a same-id stack is one logical modifier to its author), one
  // buff_modifier_removed per generation.
  addModifier(sel, mod: BuffModifierPayload, ctx): { applied: boolean; modifierRuntimeId?: string }
  removeModifier(sel, modifierId, ctx): { removed: boolean; removedRuntimeIds: readonly string[] }
  ...
  // Manual periodic trigger (spec §27/§62 + r4-review BLOCKER 1 + r5-review
  // BLOCKER 1): emits only the FIRST unit's single-request
  // PeriodicRequestsCommitted via ctx.events and stores the remaining unit
  // list as a pending continuation keyed by the emitted requestId —
  // subsequent units are emitted one-at-a-time by handlePeriodicSettled
  // (below), each computed against post-settlement state. NEVER advances
  // lifetimes, NEVER decrements turn-based modifier lifetimes. 'uses' marks
  // stay PENDING until the generated op's PeriodicOperationSettled
  // ('resolved' → consume, else release).
  // v6 — returns TriggerPeriodicStartResult (contract v7.5): start metadata
  // ONLY — later units don't exist yet at return time, so the op must not
  // claim their resolutions. {started:false} when no live unit is
  // triggerable — covers BOTH "selector matched nothing" AND "matched but
  // all candidates dead" (v7/r6 MEDIUM 1 — all-dead returns candidateUnitCount
  // = list size, no firstRequestId, no pendingSeries). candidateUnitCount =
  // the ordered list size at trigger time (candidates, NOT promised
  // resolutions — later units may skip).
  triggerPeriodic(sel, periodicId|undefined, ctx): TriggerPeriodicStartResult

  // === scheduler-registered handler — NOT a BuffAuthority port member ===
  /** v7.3/v7.4 — registered at composition as the 'periodic_operation_settled'
      immediate handler; receives the event-scoped sink.
      Bookkeeping: `pendingUses: Map<requestId, {instanceId,
      modifierRuntimeId}[]>` recorded at request computation — the mark names
      the exact modifier ENTRY (r4-review HIGH 1: `mod.${instanceId}.${n}`, a
      new generation per replace/max/min overwrite), never the bare
      modifierId. `pendingSeries: Map<requestId, {trigger, units, next}>`
      carries manual-trigger continuations.
      Status 'resolved' → decrement each marked entry's `uses` by one; at 0 the
      entry is removed AND `buff_modifier_removed` is emitted through `sink`
      (the decrement is bookkeeping; the structural removal is evented —
      r4-review HIGH 2). Anything else → release the marks. If the marked
      entry no longer exists (replaced mid-settlement) the mark dies — the
      replacement generation is untouched. Entry deleted either way; instance
      removal also drops its pending entries.
      CONTINUATION (r4-review BLOCKER 1): if `pendingSeries[event.requestId]`
      exists, walk its remaining units — revalidate `store.get(instanceId)`
      per unit, compute the next live unit's request against CURRENT state,
      mark its `uses` modifiers, emit a single-request
      `PeriodicRequestsCommitted{trigger, rootActionId: event.rootActionId,
      requests:[req]}` through `sink`, and re-key the series under the new
      requestId. Dead units are skipped; an exhausted list deletes the entry.
      The continuation chains inside the same root transaction — the emitted
      event's generated op settles in this frame and its own settled event
      drives the following unit. */
...
export type BuffEventPayload =
  ...
  | { type: 'buff_modifier_added' | 'buff_modifier_removed'; instanceId; modifierId; modifierRuntimeId }
  // v6 (r5 HIGH 1): modifierRuntimeId names the exact runtime ENTRY — same
  // authored modifierId, different generations are distinguishable in the
  // trace (reapply:'stack' entries, replace/max/min overwrites).
...
// BuffPeriodicHealRequest{requestId, instanceId, periodicId, sourceId,
// targetId, amount}.
...
Manual triggerPeriodic (r4-review BLOCKER 1 — settled-event CONTINUATION;
v6 — r5-review BLOCKER 1 truthful return):
  build the ordered unit list (canonical comparator over matched
    (instance, periodicId) pairs — the ORDER is fixed up front; payloads
    are never precomputed)
  if the list is empty → return {started:false, candidateUnitCount:0}
  const firstLive = first unit passing revalidation (store.get alive,
    instance live, target live)
  if firstLive is undefined → return {started:false, candidateUnitCount:
    <list size>}                              // v6 — r6 MEDIUM 1: all-dead
    // branch: matched candidates exist but none is live → no request, no
    // pendingSeries entry, no marks taken
  for firstLive:
    compute its request against current state, mark 'uses' modifiers
      pending (by modifierRuntimeId), emit ONE-request
      PeriodicRequestsCommitted{trigger:{type:'manual', anchorEntityId:
      instance.targetId}, rootActionId, requests:[req]} via ctx.events
  store the remainder in pendingSeries[req.requestId] = {trigger, units,next}
  return {started:true, firstRequestId: req.requestId,
          candidateUnitCount: <ordered list size>}
    // v6 — start metadata ONLY: candidateUnitCount is the candidate count at
    // trigger time, NOT promised resolutions — dead/skipped units never
    // emit. NO internal settle; the calling op's barrier drains the request
    // event → generated periodic op → PeriodicOperationSettled →
    // handlePeriodicSettled finalizes marks THEN emits the next unit's
    // request through the event-scoped sink → its op settles → its settled
    // event drives the following unit — all INSIDE the triggering op's
    // barrier.
  So `uses=1` on unit A skipped → released → unit B (computed post-release)
  folds it; A's consequence mutating modifier state → B sees the mutation.
Never advances lifetimes, never decrements turn-based modifier lifetimes
(spec §62). Pending marks/series die with the instance if removed first.
...
- Modify: `game/src/core/battle/turn/TurnBattleAdapter.ts` — construct buff2 `BuffSystem` + `CombatProcSystem` + `GaugeDeltaHandler` + registry + resolver + stats/entity/snapshot ports + `ElementalStateRegistry` + `CapabilityValidatorRegistry` at battle build; register `scheduler.registerImmediateHandler('periodic_operation_settled', (e, sink) => buffs.handlePeriodicSettled(e, sink))` (v7.3/v7.4 — the sink emits `buff_modifier_removed` on finalization-removal AND carries the manual-trigger continuation's next-request events) + the `buff_applied` gauge handler; `battleId` arrives from the composition root (R-B1)
...
| `applySkillAilments` loop `new BuffSystem(target.buffs).apply(def,...)` per stack `:2973` | ONE `ApplyBuffOperation{definitionId, stacks: ailment.stacks ?? 1, baseChance: ailment.chance, reactionEligibility:'eligible'}` through the executor — the resolver rolls ONCE (today: `this.rng() < resolveAilmentApplicationChance(...)` at `:2959` — the roll MOVES into buff2's resolver; `AilmentChance.ts` retires). **r4-review MEDIUM 1:** eligibility is `'eligible'` for every normal application — it does NOT read the legacy `canInitiateWuxingReactions` flag; whether a reaction runs is the ReactionSystem's gate (capability + canonical registry), not the producer's |
| `reactionManager.checkAndTrigger(...)` `:2979` | **REMOVED (spec §73 — no dual-run) — by reaction megaplan M-INT, scheduled inside THIS worktree (r4 BLOCKER 3 — single integration sequence).** Order inside M4: (a) data migration + consumer cutover (this step deletes the call site as part of the `applySkillAilments` rewrite — it reads AND mutates `BuffPool`, so it cannot survive the pool removal); (b) **M-INT lands on the same branch — corrected shape (r5-review BLOCKER 2):** `ElementalStateRegistry` binds elements → the five canonical ấn ids (`hoa_an`/`han_tuc`/`doc_can`/`liet_thuong`/`tran_an`) — NEVER migrated legacy ailments (`te_cong`≠`han_tuc`, `chay_mau`≠`liet_thuong`, `thach_hoa`≠`tran_an`); `TurnReactionManager` + `canInitiateWuxingReactions` + the call site delete; `elemental_reaction_enabled` is NOT granted to visible `phap_tu` (spec: only Ngộ Đạo auto-reacts — the grant is hidden-path content scope). **`ReactionDispatcher` is NOT registered at M-INT** — no valid production `ReactionRegistry` exists while `CANONICAL_REACTIONS` stays `test_*`-bound; registration lands with the seal/Ngộ Đạo mission that authors real canonical states + a production registry (validated all-10-relations) + the capability grant. **Merge gate:** M4 is not merge-ready until M-INT completes — the legacy engine/flag must be fully deleted, not left dangling (P14 verifies visible Pháp Tu does NOT auto-react). |
...
- [ ] **Step 4 — Land reaction megaplan M-INT on this worktree** (r4 BLOCKER 3 — the single integration sequence; corrected per r5-review BLOCKER 2): delete `TurnReactionManager` + `canInitiateWuxingReactions` + the call site; `ElementalStateRegistry` binds the five canonical ấn ids (never legacy ailments — `te_cong`/`chay_mau`/`thach_hoa` applications emit NO `ElementalApplicationCommitted`, so the board never sees them); `elemental_reaction_enabled` granted to NOBODY — the Ngộ Đạo grant is hidden-path content scope; **`ReactionDispatcher`/`ReactionSystem`/`ReactionRegistry` are NOT constructed in production yet** — `CANONICAL_REACTIONS` stays `test_*`-bound for unit tests, and a production registry cannot validly construct until real canonical state/payoff def ids exist (the registry's all-10-relations + `buffExists` validation has nothing legal to bind). Registration lands in the seal/Ngộ Đạo mission: author canonical states → author + validate the production `ReactionRegistry` → construct `ReactionSystem` → register `ReactionDispatcher` on `elemental_application_committed` → grant `elemental_reaction_enabled`. **Merge gate — M4 does not merge until this step is green.**
- [ ] **Step 5 — Verify (P3 FULL)** + **P13/P14 Playwright** real battle from the worktree: ailment applies, DoT ticks (damage lands before expiry), buffs expire, CC blocks a turn, and — post-M-INT — a visible-Pháp-Tu elemental application fires NO automatic reaction (spec-correct: the dispatcher doesn't exist yet; the engine activates only when canonical defs + the Ngộ Đạo grant land). **P4 deep + P5 round.**

**Exit criteria:** no production `new BuffPool()` remains except inside `BuffPersistence`; all data migrated to the new shape (zero legacy-shape defs); no `TurnReactionManager` invocation on the ailment lane AND `TurnReactionManager`/`canInitiateWuxingReactions` deleted (M-INT); canonical `ElementalStateRegistry` in place but NO production `ReactionDispatcher` registration (deferred to the seal/Ngộ Đạo mission — r5 BLOCKER 2); visible Pháp Tu no longer auto-reacts — spec-correct; full suite green; real battle verified.
...
| §34 + v7.3–v7.5 | `uses` consumed iff generated op `'resolved'` — finalized by `PeriodicOperationSettled` inside the same tree; manual trigger resolves → consumed, manual trigger skipped → survives; **manual multi-unit continuation:** A skipped → B folds the released modifier, A resolved → B does not, A's consequence mutates state → B sees it; skipped crossings release marks so a LATER crossing folds the modifier; **exact-entry identity:** same `modifierId` diff `appliedBy` / `reapply:'stack'` / mid-settlement replacement → only the marked `modifierRuntimeId` generation finalizes; **`uses:1`→0 removal emits `buff_modifier_removed` naming that runtime id**; **truthful start result (r5 B1):** 3-unit manual trigger returns `{started:true, firstRequestId, candidateUnitCount:3}` — never claims 3 resolutions synchronously; continuation emits units 2–3 only if each revalidates | `BuffPeriodic.test.ts :: manual uses finalize` / `:: manual skip retains` / `:: manual continuation sequential` / `:: pending mark entry identity` / `:: uses removal is evented` / `:: manual trigger start result` / `:: manual trigger all-dead` / `BuffLifecycle.test.ts :: interval skip releases uses` |
...
| r5 B2/B3 + r6 B2 (M-INT) | `ElementalStateRegistry` binds only the five canonical ấn ids — legacy `te_cong`/`chay_mau`/`thach_hoa` applications emit NO `ElementalApplicationCommitted` (registry says noncanonical at emit time — r5-review MEDIUM 3: the dispatcher is never reached, the gate is upstream); no `elemental_reaction_enabled` grant on the visible-`phap_tu` seam; NO production `ReactionDispatcher` registration at M-INT (no valid production `ReactionRegistry` — r5 B2) | `BuffSystemApply.test.ts :: legacy ailment emits no elemental event` / `ReactionGate.production.test.ts :: visible phap tu no reaction` / `:: no production dispatcher before canonical content` |
| r5 MEDIUM 1 | `reactionEligibility` is producer-path metadata — normal apply `'eligible'` regardless of path flags; `'suppressed'` only on recursion-suppressed lanes (reaction payoff, conversion) | `BuffSystemApply.test.ts :: eligibility is path-independent` |
...
- `ElementalApplicationCommitted` consumption + `ReactionSystem`/`ReactionDispatcher` wiring → **seal/Ngộ Đạo mission** (r5-review BLOCKER 2 — M-INT only removes the legacy engine + installs the canonical `ElementalStateRegistry`; production dispatcher registration waits for a valid production `ReactionRegistry` authored with real canonical state/payoff def ids)
...
6. RESOLVED (review rounds 3–5 — r4 BLOCKER 3 + r5 BLOCKERs 2–3 + r6 BLOCKER 2): the rollout conflict is locked to ONE sequence — reaction megaplan **M-INT** runs inside this plan's M4 worktree (after the consumer cutover that deletes the `checkAndTrigger` call site): deletes `TurnReactionManager` + the flag, binds `ElementalStateRegistry` to the five canonical ấn ids, grants `elemental_reaction_enabled` to NOBODY — and does NOT register `ReactionDispatcher` (no valid production `ReactionRegistry` until real canonical content exists — r6 BLOCKER 2; registration lands with the seal/Ngộ Đạo mission). No dangling legacy path exists — both sides of the swap land before merge.
