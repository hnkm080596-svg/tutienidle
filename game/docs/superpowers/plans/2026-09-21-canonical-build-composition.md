...
| Persisted modifier bag | `player.modifiers` (equipment via `setEquipmentModifiers` sourceType-filtered, meridian `bat-mach:*`, body `luyen-the:*` — superseded by P7-M-F (D1): body gains are flat base-stat deltas in the assembledBase, `luyen-the:*` is scrubbed and never re-emitted, realm passives, tribulation, pill permanents) | per-domain writers; attribution on `StatModifier.sourceType` |
