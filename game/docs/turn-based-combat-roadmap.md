...
| ~~Ultimate-as-loadout-slot~~ — **GỘP vào "Manual tap-to-cast UI" bên dưới (2026-09-04, phiên khảo sát 4)**: khảo sát sâu hơn xác nhận Ultimate hoàn toàn KHÔNG phải hệ riêng trong turn-based — chỉ là 1 skill bình thường gắn tag `ultimate`, hoạt động y hệt 5 slot loadout khác (không nút riêng/auto-toggle riêng/resource-gating đặc biệt — dùng chung cơ chế resource/cooldown mà slot 0-4 dùng). Quyết định duy nhất còn lại: slot index 5 (ô thứ 6) dành riêng, không hoán đổi được, cho skill path-default gắn tag `ultimate`. `UltimateSystem.ts`/`BattleSystem.tryPlayerUltimate()`/`KiemTuCombatHud.vue`'s nút Ult retire hoàn toàn khi cutover — không có mảnh nào tách ra xây riêng trước đó (không liên quan turn/thời gian, thuần là UI/wiring, phải làm chung 1 lần với Manual UI) | Quyết định chốt trong phiên brainstorm 2026-09-04 (khảo sát 4) | _(gộp — xem dòng Manual tap-to-cast UI)_ | 🟢 Đã quyết định: không phải hệ riêng, gộp thiết kế + implementation vào Manual UI |
...
| Manual tap-to-cast UI (`CombatSkillSlot.vue` và liên quan) — hiện HOÀN TOÀN chưa tồn tại (0 UI thủ công), người chơi chỉ chọn skill, không chọn target (targeting luôn tự động — [spec §4.5](../../docs/superpowers/specs/2026-09-03-turn-based-combat-design.md)). **Nay gồm cả Ultimate** (xem dòng gạch trên): slot index 5 dành cho skill tag `ultimate`, cast y hệt 5 slot kia, retire `UltimateSystem.ts`/`KiemTuCombatHud.vue`'s nút Ult riêng khi thiết kế/implement UI này | Đã chốt về hành vi (spec §4.5) + Ultimate gộp vào (2026-09-04), UI cụ thể chưa thiết kế | _(chưa viết)_ | 🔴 Chưa khảo sát/quyết định |
