...
| `.claude/worktrees/phap-tu-thuan-he` (`worktree-phap-tu-thuan-he`) | Pháp Tu Thuần Hệ — 20 skill chuỗi B–E + 5 ult per-element + node tree 17 node/hành + 8 engine ext (E-1..E-8) + glue chain/ult + tooltip cơ chế (plan `2026-09-03-phap-tu-thuan-he.md`, spec `newPhapTuDesignSpec.md` §0.b) | 🟡 **Code 100% commit (18 commits, Task 1–12b) + docs sync** — 2026-09-03. Verify xanh: type-check, full suite 2318/2318, build, e2e boot-fresh + create-to-combat. QA quick **PASS WITH GAPS** (`game/docs/qa/2026-09-03-phap-tu-thuan-he-task12-quick.md` — 1 Coverage gap Medium: thiếu nút ult thủ công trong `PhapTuCombatHud.vue`, engine đã sẵn qua `tryPlayerUltimate()`; 1 Suspected Low: Thế không tiêu được khi farm không boss). KHÔNG merge — chờ user. |
