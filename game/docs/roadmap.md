...
| — | Buff System Reimagined (`core/buff2`, spec `2026-09-17-buff-system-reimagined-spec.md`) | buff engine | 🟡 COMPLETE on `feat/buff-core` (merge-ready) — M0 inventory → M1 types/store/query/registry → M2 modifier engine + resolver + authority surface → M3 lifecycle/periodic/persistence → M4 data+consumer cutover (`d65f28aa`: scheduler-owned mutation ops, capability owners proc/marker/theTu/gauge/dotRecovery, persistent `BuffPersistence`, entry-buff post-mint lane, M-INT landed inside) → M5 legacy `core/buff/` + `AilmentChance` + dormant ArtifactSystem/Battle cluster deleted, spec DoD sweep (`BuffAcceptance.test.ts` §70–72, `ReactionGate.production.test.ts` M-INT inert), `docs/systems/buffs.md` rewritten |
| — | Reaction System Reimagined (`core/reaction`, spec `2026-09-17-reaction-system-reimagined-spec.md`) | reaction engine | 🟡 IN PROGRESS on `feat/reaction-core` — M0 inventory → M1 gate/board/registry → M2 candidates+selection → M3 snapshot/batch semantics → M4 payoff emission + canonical data + Cấm Công → M5 dispatcher (registration deferred)/trace/determinism DONE; M-INT LANDED inside buff M4 (`d65f28aa` — `TurnReactionManager`/`canInitiateWuxingReactions` deleted, canonical `ElementalStateRegistry` bound, zero dispatcher registration). Engine stays production-inert: no capability grant, no `ReactionDispatcher` registration until canonical ấn content lands (seal/Ngộ Đạo mission) |
