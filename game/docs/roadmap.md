...
| — | Combat Systems Contract — shared operation/event/scheduler spine (`core/battle/contracts` + `core/battle/runtime`, spec `2026-09-17-combat-systems-contract-spec.md`, megaplan `2026-09-17-megaplan-combat-contract`) | contract infra | ✅ LANDED on `feat/combat-contract` — M0 census → M1 contracts → M2 scheduler/executor/batch/trace → M3 current-engine adapters → M4 per-cycle `CombatRng` composition root → M5 trace exporter + DoD sweep (`docs/architecture/2026-09-17-combat-contract-dod.md`). **Dormant:** scheduler minted per battle, zero authored ops routed, `buffs` port undefined until the buff megaplan; sibling plans (skill / reaction / buff, seal batch) consume the layer |
| — | Buff System Reimagined (`core/buff2`, spec `2026-09-17-buff-system-reimagined-spec.md`) | buff engine | 🟡 COMPLETE on `feat/buff-core` (merge-ready) — M0 inventory → M1 types/store/query/registry → M2 modifier engine + resolver + authority surface → M3 lifecycle/periodic/persistence → M4 data+consumer cutover (`d65f28aa`: scheduler-owned mutation ops, capability owners proc/marker/theTu/gauge/dotRecovery, persistent `BuffPersistence`, entry-buff post-mint lane, M-INT landed inside) → M5 legacy `core/buff/` + `AilmentChance` + dormant ArtifactSystem/Battle cluster deleted, spec DoD sweep (`BuffAcceptance.test.ts` §70–72, `ReactionGate.unwired.test.ts` — renamed from `ReactionGate.production.test.ts` once production wiring landed), `docs/systems/buffs.md` rewritten |
| — | Reaction System Reimagined (`core/reaction`, spec `2026-09-17-reaction-system-reimagined-spec.md`) | reaction engine | ✅ LIVE — M0 inventory → M1 gate/board/registry → M2 candidates+selection → M3 snapshot/batch semantics → M4 payoff emission + canonical data + Cấm Công → M5 dispatcher/trace/determinism → M-INT legacy deletion (`d65f28aa` — `TurnReactionManager`/`canInitiateWuxingReactions`/`ELEMENT_REACTIONS` deleted, canonical `ElementalStateRegistry` bound) → **canonical-seals/Ngộ Đạo batch** (`feat/canonical-seals-ngo-dao-reaction`): 5 seals (`hoa_an`/`doc_can`/`liet_thuong`/`han_tuc`/`tran_an`) thay legacy ailments, `CANONICAL_REACTIONS` bind production payoff defs (`ReactionStatusBuffs.ts`), `ReactionDispatcher` registered trên `elemental_application_committed` trong `GameManagerTurnBattleOps`, `elemental_reaction_enabled` granted party-wide qua aura `van_phap_than_hoa`, `reaction_damage` sealed vào `reaction` channel |
| — | Skill Definition System — one ACTIVE turn-combat skill pipeline (`core/skilldef`: `SkillDefinition` → `SkillResolver` → `ResolvedSkillPlan` → `SkillExecutor` → `CombatScheduler`; spec `2026-09-17-skill-definition-system-spec.md`, megaplan `2026-09-17-megaplan-skill-definition.md`) | skill engine | 🟡 IN PROGRESS on `feat/skill-definition` — M0 inventory (`docs/architecture/2026-09-17-skilldef-inventory.md`) → M1 schema + 3-state split (`SkillDefinition`/`SkillProgressionState`/`SkillCombatRuntimeState`) + contract v1.6 → M2 `CastSnapshot`/`SkillResolver`/plan IR → M3 `SkillExecutor` + query/commit ports + subcast driving → M4 plan routing: adapter-covered casts execute through the scheduler (`LegacySkillAdapter` bridge, `TurnSkillPlanRuntime`, `SkillExecutionHooks` orchestration seam) → M5 legacy-lane retirement (charge/dynamicBasic extras/composite extras routed; unsupported+runtime = loud no-op; runtime-absent = documented engine-unit lane), `SkillToTurnSkillConverter` retired into `LegacySkillAdapter`, `SkillSystem.progressionOf`/`getResolvedSkill` + combat-runtime projection seams, spec §73 DoD sweep. Authored skill content is scheduled for redesign ON SkillDefinition — legacy defs ride the adapter meanwhile |
| — | Combat Systems Reimagined — Final Battle Wiring & Contract Closure (megaplan `2026-09-19-megaplan-final-battle-wiring.md`; inventory `docs/architecture/2026-09-21-m7-final-battle-wiring-inventory.md`) | wiring/contract closure | 🟡 COMPLETE on `feat/m7-battle-wiring` (merge-ready, pending user merge) — M7.0 inventory → M7.1 authority/composition closure (single `writeExternalWardGrant` seam, engine-lane dead code removed, production `TurnBattleSystem` construction guard) → M7.2 closure matrix (`docs/qa/2026-09-21-m7-contract-closure-matrix.md`, spec status FINAL — ACTIVE CONTRACT) → M7.3 whole-stack acceptance suite (`TurnBattleSystem.contract.test.ts` §§91–102 + skill stack; §49 per-op validity gate unified into shared `CombatOperationBatchRunner`; fixture `setRandomSource` bind) → M7.4 determinism/trace (`TurnBattleSystem.determinism.test.ts` same-seed parity + divergence + causal-graph reconstruction; `FixtureReaction` shared composition) → M7.5 journeys (`GameManager.skillPipelineJourney.test.ts` real doc_chuong/An-kit/charge content), legacy deletion sweep (engine lane test-only, construction-guarded), CON-01..23 deep evidence (`docs/qa/2026-09-21-m7-contract-closure-deep.md`), Playwright real-battle PASS. Gates: `npm run verify` 683f/5821t green, P18 OCR 100% (1 Low fixed), P4 deep QA PASS WITH EVIDENCE (`docs/qa/2026-09-19-m7-battle-wiring-deep.md`), P5 sequential review clean. **Runtime architecture CLOSED** — production Reaction/content activation đã land trong canonical-seals/Ngộ Đạo batch (`feat/canonical-seals-ngo-dao-reaction`: dispatcher registration + Ngộ Đạo grant + payoff defs + Cấm Công production buff — xem Reaction System Reimagined row) |
...
> Per-way Playwright matrix (M10): DONE post-merge —
> `tests/e2e/cultivation-path-ritual.spec.ts`, 7/7 green (gate-negative
> + all six ways through the real tribulation loop; includes the
> ngo_dao no-artifact / phap_bao-slot-disabled oracle).
...
- **GameManager 2.939 dòng**: ~~đợt tách Ops mới~~ ✅ **XONG (large-file split, branch `feat/large-file-split`)** — `GameManager.ts` 3174→1173 (Catalog/Reward/Progression/RealmAdvance/PersistentEffect/Economy/Pill/Tick Ops + migrate call site thẳng sang `gm.<domain>Ops.*`, bỏ facade), `GameManagerTurnBattleOps` 1719→1308 (auto-farm/battle-reward/presentation Ops con), `CombatScene` 2832→2154 (telegraph/snapshot-reconcile/animation-playback/action-feedback), data barrels Skills/Enemies/PhapTuNodes/buffs, `EquipmentSystem` 1507→886, `ProductionSystem` 836→626, `SaveSystem` → tách `saveTypes.ts`, standalone panels trong `GameRoot` lazy-mount-once (`defineAsyncComponent` + v-if). `TurnBattleSystem` (1319) giữ nguyên — state graph đan xen scheduling↔resolution, tách máy móc sẽ tạo god-deps. `spawnEnemy` facade chết đã xoá. `check:bundle-split` contract giữ nguyên (entry 587KB<900, phaser riêng, 20 chunks — các panel thành chunk lazy). `vitalsWriteAuthority` allowlist cập nhật: dead-spawn `entity.alive=false` chuyển sang `GameManagerAutoFarmOps`.
