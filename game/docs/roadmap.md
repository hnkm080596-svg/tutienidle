...
| — | Buff System Reimagined (`core/buff2`, spec `2026-09-17-buff-system-reimagined-spec.md`) | buff engine | 🟡 IN PROGRESS on `feat/buff-core` — M0 inventory → M1 types/store/query/registry → M2 modifier engine + resolver + authority surface → M3 lifecycle/periodic/persistence DONE; M4 data+consumer cutover pending (shares worktree with Reaction M-INT) |
| — | Reaction System Reimagined (`core/reaction`, spec `2026-09-17-reaction-system-reimagined-spec.md`) | reaction engine | 🟡 IN PROGRESS on `feat/reaction-core` — M0 inventory → M1 gate/board/registry → M2 candidates+selection → M3 snapshot/batch semantics → M4 payoff emission + canonical data + Cấm Công → M5 dispatcher (registration deferred)/trace/determinism DONE; M6 + M-INT pending. Engine production-inert AND unwired: no capability grant, no `ReactionDispatcher` registration until canonical ấn content lands (seal/Ngộ Đạo mission). Legacy `TurnReactionManager` stays live until M-INT deletes it inside the buff M4 worktree |
