...
| M-QI-09 | DONE | `7916fbe9` | verify full (type-check + build + 741 files / 6637 pass; 3 failures pre-existing env `magick` ENOENT) + OCR (9 reviewable, 100% coverage) + deep QA report (`docs/qa/2026-09-23-mqi-09-essence-substitution-deep.md`, PASS WITH GAPS — one Suspected/Low internal-precondition item) + P5 sequential 3 passes — `PHYSIQUE_ESSENCE_CONVERSION_RATIO = { bao: 2, phap: 2 }` sim-locked (authored-parity surplus: residual 36,790 = Σ tier caps ≈ one residency at pinned 1.4 essence/kill; smallest candidate ≥ 2 binds); `BodyChapterEssenceSubstitution` pure resolver (per-hop integer product, lowest-grade-first ascending minimum-whole-units, exact change-back in required units); `investBodyChapter` widens availability to owned + coverage, preflights every debit via `bag.has` before mutation, all-or-nothing commit; `EssenceSubstitutionEconomy` sim — candidates → deterministic selection → production write → regression test reads PRODUCTION data; `measureStrandedCompletion` retires all 6 tiers at `ceil(residual/2)=18,395` Bảo on seeds 11/22/33/44 and fails at budget−1; namespace gate inside resolver (typed cost in; pill/foreign ids refused); C2C round-10 rulings 1-6 applied (per-hop floor math, min-unit+exact-change, apply-side preflight, sim ordering, no-arb data test, resolver-side gate); C2C impl round-17 rulings applied (strict validate→consume→apply via detached plain-data probe (JSON round-trip — never `structuredClone`, reactive Pinia `$state` callers; C2C r24) + fail-closed commit; change-credit stack-capacity preflight aborts atomically; downstream-hop rule pinned — `phap === lockedRatio` measured, Pháp→Phàm compound exercised e2e); no live drop-band swap (M-QI-10), no exchange UI, downward-only, no migration (QI-S) |
| M-QI-10 | DONE | `d6eae631` | verify full (type-check + build + 745 files / 6734 pass; 6 failures all pre-existing base/env: 3 SkillPathPanel `getTurnBattle` mocks from 04364847, 2 `magick` ENOENT, 1 asciiComments on sibling M-F-RESPEC files — zero task-owned) + scoped sweep (41 files / 324 tests) + OCR (6 reviewable, 100%) + quick QA report (`docs/qa/2026-09-23-mqi-10-essence-drop-band-quick.md`, PASS WITH EVIDENCE) + P5 sequential 3 passes — `STAGE_DROP_TABLES` banded realms carry `PHYSIQUE_ESSENCE_BAND_DROPS[realm]` by reference (mortal literal replaced, LQ→Bảo, TC→Pháp live at pinned 0.7 / 1-3); identity + stray-lane + runtime-emit + gate-miss pins; `huyet_mong` Phàm ×12 signature exempt; sink invariant derives essence sinks from real substitution yield (QI-D4b/QI-S) |
| M-QI-11 | DONE | `6a2e9985` + `34be05e0` + `8178c3e5` | landed via the kiem-tu orbit: 37-combo authored table, suffix-free guard + modifier contract, hien dynamic-basic wiring through the engine, external-review fix batch — per `git log` on `p7/truc-co` |
| M-QI-12 | COVERED | — | scope subsumed by M-F-JOURNEY legs A–B (initiation → chapter-10 → L12 → breakthrough on `TrucCoJourney.test.ts` real seams); disposition per m-f-journey spec §9 — no separate suite |

## Trúc Cơ Missions (M-F)

Twelve-mission block landing the Trúc Cơ surface on `p7/truc-co`
(specs/plans under `docs/specs|plans/m-f-*`; per-mission external
C2C review). Authority boundaries per landed spec docs; save bumps
v75–v81.

## M-F completion ledger

| # | Status | Commit | Gates |
|---|---|---|---|
| M-F-REALM18 | DONE | `365c804a` | realm-18 normalization for Kim Đan+ (PR #2) |
| M-F-RESPEC | DONE | `04364847` | player node respec (PR #3) |
| M-F-CEILING | DONE | `4b8d8270` | release-policy authority — `progressionCeilingRealmId` single ceiling constant (PR #4) |
| M-F-BODY-CORE | DONE | `41580c17` | chapter-kind + physique seam + base-stat collector channel (PR #6) |
| M-F-TECHNIQUE | DONE | `271df96d` | frozen-cycle technique model; `gradeHistory` + rank/grade contract; save v75 (PR #10) |
| M-F-TALENT | DONE | `c40e72cb` | mandatory breakthrough talent transaction; `pendingTalentEntitlement` locks settled breakthroughs until resolved; save v76 (PR #9) |
| M-F-ESSENCE | DONE | `3b5fbac5` | TC-side essence contract pins — Pháp band `0.7` / `1–3` on every TC stage table (PR #11) |
| M-F-COMPANION-GIFT | DONE | `f9489e7b` | beta companion gift mail — `companionGifts` + `claimCompanionGift` (`gift_than_nong_foundation_entry`, `gift_khai_minh_foundation_floor_10`); save v77 (PR #12) |
| M-F-CHU-THIEN | DONE | `fc78bc23` | `zhou_tian` body chapter — capacity `min(360, 20×realmLevel)`, Tiểu 180 / Đại 360, meridian-complete gate; save v78 (PR #13) |
| M-F-ARTIFACT-DEFER | DONE | `859ebc01` | artifact domain deferred to Kim Đan+ — `ARTIFACT_UNLOCK_REALM_ID = 'golden_core'`; save v79 (PR #15) |
| M-F-BODY-PERFECTION | DONE | `f9fc4ee4` | hidden body-perfection pipeline — `bodyPerfection` slice; registry stays empty until the content pass (negative leg); save v80 (PR #14) |
| M-F-BODY-HIDDEN | DONE | `0f88375a` | generalized hidden material channels — `HIDDEN_MATERIAL_CHANNELS` (huyết-mông `hidden_beast` qi_refining band), persisted `hiddenBeastKills`/`hiddenChannelCycles`; save v81 (PR #17) |
| M-F-JOURNEY | IMPL | this branch | ordered TC E2E journey + integration sweep + docs sync + save-version decision (v81 stands); notes `docs/p7/missions/mf-journey.notes.md` |
