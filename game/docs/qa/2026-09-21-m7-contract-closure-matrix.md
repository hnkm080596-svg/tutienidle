...
### M7.5a/b real-content journey evidence

`src/core/game/GameManager.skillPipelineJourney.test.ts` drives the full production composition (`GameManager` → `beginBattleCycle` → `mintCycleScheduler` → `TurnBattleSystem` + plan runtime) on REAL authored content lifted through the strict converter:

- **Journey 1** — `phap_tu`/`ngu_hanh`/wood: authored `doc_chuong` (0 direct damage, `trung_doc` chance 1) applies the ailment via `BuffSystem`, the periodic DoT settles damage through `CombatScheduler` → `DamageSystem`, the enemy dies, the stage reports `victory`. Because the authored skill cannot deal direct damage, all enemy HP loss is attributable to the buff-periodic lane.
- **Journey 2** — `phap_tu`/`ngo_dao` via the real ritual: authored `da_phap_lien_tuyen` (composite + `repeatCasts: 2`) logs exactly 3 executions per cast under the root id while `skillCastCounts` increments once and the slot cooldown never re-bumps; authored `van_phap_tuy_tam` (composite + `multicast` via the innate dao passive) produces executions outnumbering casts.
- **Journey 3** — companion `van_du_kiem_khach` ultimate authored with `chargeTurns: 2`: charge init commits the cooldown once, `chargingTurnsRemaining` counts 2 → 1 across the companion's own turns with no phantom actions mid-charge, the deferred resolve lands the authored hit and does NOT re-initiate a cast (`charging`/`pendingChargedSkillId` clear rather than resetting to 2), and normal actions resume afterward.
- All three journeys assert zero `console.warn` unsupported-semantics / strict-converter-rejection / partial-execution reports — nothing silently dropped, nothing fell back off the canonical lane.

### M7.5c deletion/legacy sweep result

Swept `TurnReactionManager`, `canInitiateWuxingReactions`/`wuxing`, `BuffPool`, `SkillToTurnSkillConverter`/converter layers, silent runtime fallback, cross-authority mutation helpers, per-skill/per-ID handler lanes, and any competing active-skill execution representation:

- `TurnReactionManager`, `BuffPool`, `SkillToTurnSkillConverter`: no live code — file/symbol search returns comments and historical references only. `WuxingRelations` is the legitimate element-relations content system, not the retired reaction manager.
- The `runtime === undefined` engine lane is the one remaining legacy execution representation and stays test-only: production `TurnBattleSystem` constructions are now guarded by `tests/architecture/damageAuthorityRng.test.ts` — every production site must inject an explicit `CombatRng` (8th arg) and **at most one** site may omit the `TurnCombatRuntime` (5th arg; the documented inert bootstrap placeholder, structurally unreachable because `stepTurnBattle` early-returns while `turnBattle` is unset).
- Runtime-present casts that cannot route are loud no-ops via `reportUnroutedCast` (warn once per cast identity per battle); every `this.runtime`-gated accessor throws rather than silently degrading; no silent fallback exists.
- Cross-authority direct writes re-audited: all remaining `currentHp`/`alive`/`currentWard`/`currentMp`/`currentThe`/`actionGauge` writes sit inside owning authorities (`EntityVitalsSystem`, `CombatSystem` kill/vitals, `ActionGauge`, `EntityResourceAdapter`, `TheEconomy`) or the engine-unit lane's own engine implementation (`commitCast`/`grantTheFromCast` — unreachable in production), plus documented init/restore lanes (`Player`, `TribulationDirector` ghost snapshot, `GameManagerAutoFarmOps` offline sim).
- No per-skill/per-ID handler lanes: all `skill.id ===` comparisons are slot/identity matching (charge-id match, NULL_ACTION `''`, basic identity), never content dispatch.
- Production Reaction remains inert: no `new ReactionDispatcher`/`ReactionSystem`/`ReactionRegistry` outside test/fixture code; `elemental_reaction_enabled` is never granted by any production buff definition.
- `BuffPersistence` synthetic `combatSequence` is the documented out-of-battle persistent-lane exception — its constructor rejects periodic defs (no settlement barrier), and its counter never enters a battle trace.
