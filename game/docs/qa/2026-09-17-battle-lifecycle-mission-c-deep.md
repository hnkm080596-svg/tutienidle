...
### EXT-C13 (LOW, follow-up): successful mid-fight replace ended the old battle with no terminal
- **Evidence:** C11 established "a mid-fight replace is an implicit battle end" and banked the carry - but the terminal emit only existed on the FAILURE path (discard). A successful replace banked A's carry, swapped `this.turnBattle` to B, and never published A's `battle_end` - violating the rewardOps exactly-one-terminal contract for presentation consumers.
- **Fix:** the implicit-end block in `beginBattleCycle` now performs the full abandon-equivalent terminal pre-commit: bank carry (bound player only) -> mark defeat -> `emitAbandonEnd()` -> release the stage run via `stopRepeat()`, guarded on the incoming cycle's kind (a 'stage' launch already acquired the slot for ITS stage - releasing there would kill the new battle's own lease; 'repeat' preserves its run's lease and is unreachable anyway since its previous is always terminal). The discard shrank accordingly - no parameter, no terminal branch: by the time it runs the previous battle is always already terminal, and the half-built new battle is dropped silently as before.
- **Side effect:** the conditional `stopRepeat()` also closes the previously documented residual - a successful fresh/test replace of a live STAGE battle now releases the wave lease instead of soft-locking the slot.
- **Test:** `stageLease.test.ts` C13 - successful replace publishes `['defeat']` for the outgoing battle and the new battle runs. Red pre-fix (`[]`).
- **Verification:** type-check clean; scoped 115 files / 686 tests (4 expected-fail).
- **Direction note:** the reviewer offered reject-vs-formalize. Implicit live-replacement is the pervasive test-suite pattern and the dev seam, so the formalized exactly-once terminal was implemented; rejecting non-terminal replacement would invalidate the C11/C12 failure-contract surface the round-3 review had just demanded.
