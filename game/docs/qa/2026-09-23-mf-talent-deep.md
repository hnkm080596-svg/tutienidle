...
## C2C Impl Review Round 42 — fixes applied

Coordinator-verified findings (2 High + 1 Medium), all confirmed real; fixed on `devin/1790133063-m-f-talent`:

- **C2C-42-1 (High): Đại Đạo breakthrough produced TWO results** — `resolveVictory` minted a generic entitlement AND ran the `pham_cot` → `pham_nhan_chi_cot` conversion on the same seam. Fix: `isGreatDaoBreakthrough` (`targetRealmId === 'foundation_establishment' && grade === 'great_dao'`) computed once; the generic mint is suppressed on that path so the transaction yields exactly one result — the evolution, which stays the explicit special case per the ruling. The conversion reuses the same predicate (single source of the special case). Regression: OutcomeService tests assert `pendingTalentEntitlement === undefined` on the great_dao path + conversion still applies; a `heaven`-grade foundation victory still mints the tc pool entitlement; the dotPha great_dao spec pins no-lock drain.
- **C2C-42-2 (High): resolution trusted the persisted record** — `resolveTalentEntitlement` rechecked nothing beyond includes/ownership/definition, so a shaped save could carry any catalog id or an unreleased-pool entitlement; and a record whose offers were all already-owned + no upgradeables left the uncancellable modal unresolvable. Fix: NEW decisions now re-enforce `isBreakthroughAcquisitionEnabled(entitlement.realmId)` + `offeredTalentIds ⊆ BREAKTHROUGH_TALENT_POOLS[realmId]` (the same origination authority). New domain predicates `isTalentEntitlementActionable` / `reconcileTalentEntitlement` clear a record that presents zero legal decisions — wired at the drain lock (`checkTribulationOutcomeAction`) and defensively in the modal's `watchEffect`; the load validator's degenerate guard now delegates to the same domain check (covers all-owned offers + release-suppressed pools, not just empty offers). Regression: non-pool offer rejected, dormant-pool offer rejected, all-owned/dormant records reconcile away, valid records survive.
- **C2C-42-3 (Medium): latent unowned `talentLevels` bypassed level 1** — NEW grant pushed the id to `selectedTalentIds` leaving a shaped `talentLevels[x]=3` live. Fix: NEW grant writes `talentLevels[id] = 1` (level-1 contract); validator fails `talentLevels` keys not present in `selectedTalentIds`; the great_dao splice now deletes the orphaned `pham_cot` level entry. Regression: domain grant-normalize test; validator latent-unowned fail test; retired-but-owned level stays tolerated.

Verification after fixes: `npm run type-check` clean; `npm run build` green; scoped vitest 692/692 (talent + tribulation + save + composables) and 1379/1382 across the full affected scope — the 3 failures are the pre-existing `SkillPathPanel` test-double defect (`getTurnBattle`), identical on base. P15 TOTAL_NEW 0.

## C2C Fix-Verification Round 47 — fixes applied

Coordinator-verified Medium, confirmed real; fixed on `devin/1790133063-m-f-talent`:

- **C2C-47 (Medium): offer-set integrity was existential-only** — `isTalentEntitlementActionable` passed when SOME offered id was legal, so a persisted mixed set (e.g. `[lk_bac_hai, tc_dia_can]` under `qi_refining`) validated; the modal then mapped every resolvable id to a card, painting foreign/dead choices the hardened resolver would reject; and actionability/resolution treated weight-0 pool members as grantable though the draw can never produce them. Fix per coordinator pin — ONE canonical predicate `isLegalBreakthroughOffer(realmId, talentId)` (realm-pool member AND `weight > 0` AND catalog-resolvable; ownership excluded — callers check it where it matters; release gate stays separate) now shared by origination (`drawBreakthroughTalentOffers` delegates), actionability (`hasLegalNewOffer`), resolution (NEW branch re-enforcement), and save preflight. Preflight requires EVERY persisted offered id to satisfy it — foreign/zero-weight ids fail loud at `offeredTalentIds[i]`; the degenerate/actionability question (≥1 still-unowned offer or a legal UPGRADE) stays with `isTalentEntitlementActionable`. The modal's `offeredTalents` filters the persisted set through the same predicate plus decision-time ownership — no dead cards.
- Regression tests: `TalentEntitlement.offerLegality.test.ts` mocks the pools to pin the weight-0 member case (predicate false, draw never produces it, resolution rejects it, record non-actionable + reconciles, mixed-set: legal grants / dead rejected); `TalentEntitlementModal.deadCards.test.ts` mounts the real component (jsdom) and asserts foreign/retired/owned ids never paint and an all-dead record reconciles to no dialog; `saveShapeValidation.test.ts` gains the mixed-set rejection case (`lk_bac_hai` + foreign `tc_dia_can` → fail at index 1).

Verification after fixes: `npm run type-check` clean; `npm run build` green; scoped vitest 514/514 (talent + common components + save validation) and 80/80 (tribulation + useTribulation + layout).
