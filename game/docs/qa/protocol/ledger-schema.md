...
14. (v2) Prevention records: `run.briefIds` resolve to briefs; a brief declaring `IMPLEMENTATION_READY` has empty `unresolvedAssumptions` and no `STALE`/`NEEDS_DISCOVERY` routing; assignment history is chronological and continuous (`history[i-1].to == history[i].from`); a `FINISHED` assignment carries `releaseEvidence` (a result message is not a release); consumptions resolve to `lessonId@version` and a known brief; a `QUALIFIED` guidance facet has `authorityRefs`, `qualificationEvidence` and independent `qualifiedBy`; `causalPrevention` beyond `NOT_ESTABLISHED` carries supporting evidence; a PROMOTED lesson may not carry a non-QUALIFIED facet (facets publish independently of detection promotion).
...
## Schema version 2 (prevention upgrade, 2026-09-24)

`ledger.schema.json` is now `urn:tutienidle:internal-qa:ledger:2`. The frozen v1 schema is preserved verbatim at `game/scripts/qa/ledger.schema.v1.json` — historical v1 ledgers validate and replay against v1 unchanged; `validate` selects the schema by `ledger.schemaVersion`. Version 1 -> 2 migration is **explicit** (`qa:internal migrate --run <id>`) and is permitted only for a run that RESUMES; it appends a `MIGRATION` event, initializes empty `briefs`/`assignments`/`consumptions`, sets `run.capacityLimit` (default 5) and `run.requiredReadiness = false` (a v1 run never met v2 obligations — no retroactive claim), and leaves existing lessons `guidance: null`. Nothing fabricates evidence during migration.

New record families (full shape in the canonical JSON `$defs`):

- `brief` — the construction brief: task identity, `planningBaseline` state, `policyHash`, bound `requirementRefs`/`invariantIds`/`dependencyRefs`/`sourceDependencyHashes`, `lessonRouting` records, ordered `slices`, `firstProofObligations` (oracle + `BEFORE_WRITE`/`BEFORE_PROPAGATION` stage + admitted slice), `unresolvedAssumptions`, readiness (`DRAFT`/`IMPLEMENTATION_READY`/`DISCOVERY_REQUIRED`/`SCOPE_DECISION_REQUIRED`/`STALE`), `preflightEvidenceIds`, `deviations`, `consumedBy`, `checkpoints`, `finalConformanceIds`.
- `assignment` — a slot-reservation record for dispatched agents: capacity scope, immutable `inputBundleHash`, readiness (`READY`/`WAITING_INPUT`/`BLOCKED`), status machine `QUEUED/READY/RESERVED/RUNNING/RESULT_RECEIVED/RELEASE_PENDING/FINISHED/BLOCKED/CANCELLED`, `reservedAt`, `observedRuntimeId`, `releaseEvidence`, `timeoutState`, chronological `history`.
- `consumption` — proof a published guidance facet was consumed on an ordinary task: `lessonRef`, `briefId`, `policyHash`, `actionTaken` (`APPLIED`/`REJECTED_WITH_PROOF`/`STALE_REJECTED`/`DEVIATION_RECORDED`), `observedOutcome` (`EARLY_CORRECTION`/`SEAM_DEFECT_FOUND`/`APPLIED_NO_DEFECT`/`ESCAPE_OBSERVED`/`NONE`), evidence ids.
- `guidanceFacet` on a lesson — the prevention payload (see `learning.md`); independently qualified and published into `learning/policies/<hash>.json` addressed by content hash.
- `checkpoint` inside a brief — baseline vs observed state, `invalidatedItems`, `nextSlice`, reason; state drift marks the brief STALE.
- `run` gains `briefIds`, `capacityLimit`, `requiredReadiness`; `event.kind` gains `MIGRATION` and `SCHEDULE`.

A v2 run with `requiredReadiness` cannot reach `QA_FIXED_POINT_REACHED` while a declared brief never reached `IMPLEMENTATION_READY` or lacks final conformance evidence (decision clause `C9-readiness`).
