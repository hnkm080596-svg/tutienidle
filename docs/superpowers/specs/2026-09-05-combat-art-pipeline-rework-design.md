# Combat Art Pipeline Rework — Entity Sync + Sprite Sheet Animation + Battlefield Region Design
...
4. **Battlefield region**: a new `BattlefieldUsableRegion` concept constrains WHERE entities may be placed/spawned. It does NOT change `GRID_ROW_COUNT`/`GRID_COLUMN_COUNT` or any distance/targeting/AoE formula — those keep operating on the full 10×16 coordinate space. Exact bounds are supplied later by the user; the mechanism defaults to "the full grid" (i.e., zero behavior change) until real bounds are provided.
...
export const DEFAULT_BATTLEFIELD_USABLE_REGION: BattlefieldUsableRegion = {
  rowMin: 0,
  rowMax: GRID_ROW_COUNT - 1,
  columnMin: 0,
  columnMax: GRID_COLUMN_COUNT - 1,
} // full grid — zero behavior change until real bounds are supplied
```

(If the real region turns out not to be a simple rectangle, this shape gets revisited when the user supplies the actual bounds — flagged as a risk in §8, not decided now.)
...
- The exact `BattlefieldUsableRegion` bounds — user supplies these later; this spec ships the mechanism with a full-grid default.
- VFX authoring, camera work, hit-stop polish — already explicitly out of scope per the Action Playback spec, unchanged here.
...
- `BattlefieldUsableRegion`'s rectangle-only shape is an assumption; if the user's eventual bounds are an irregular shape (e.g. an L-shaped playable area), the plan/executor should flag this back rather than silently forcing it into a rectangle.
