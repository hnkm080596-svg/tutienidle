...
**Boss always fights solo (2026-09-05 addendum)** — a boss stage has EXACTLY ONE enemy total (the boss), never regular enemies alongside or before it. Today `StageWaveSystem.pickEnemyForSpawn()` already special-cases `stage.totalEnemyCount === 1` to spawn the boss immediately as the only/final enemy (`StageWaveSystem.ts:66-68`) — the fix is a small `effectiveTotalEnemyCount(stage)` helper used everywhere `stage.totalEnemyCount` currently feeds wave/spawn/win-condition config, returning `1` whenever `stage.floor === 10 && stage.bossEnemyId` is set (matching `pickEnemyForSpawn()`'s own existing boss-gate condition, `StageWaveSystem.ts:260`) regardless of what raw `totalEnemyCount` a stage's content data happens to specify — this makes "boss solo" a systemic guarantee rather than a per-stage content authoring convention that could silently be violated.

**Boss hitbox — no new mechanism needed.** The engine's targeting model (`selectTarget()`/`collectTurnTargets()`, `BattleGrid.ts`/`TurnSkillAction.ts`) always resolves an attack against the SET of currently-living opposing entities, never a raycast against empty grid cells — there is no way to "miss" by aiming at an unoccupied cell. Once boss-solo (above) guarantees the boss is the only living entity in `battle.enemies`, every `selectTarget()`/AoE resolution trivially always resolves to the boss regardless of the attacker's own position or the AoE shape's exact radius — this is what the user's "hitbox full 6x6" requirement actually needs, achieved for free by the solo guarantee. No change to any targeting/distance formula.

**Boss sprite is 2× the size of a regular enemy sprite (2026-09-05 addendum)** — today ALL enemies (including boss) already render at `ENEMY_DISPLAY_SCALE_MULTIPLIER = 2` (`combatConstants.ts:80`), confirmed explicitly intentional by an existing locked test (`CombatScene.enemyScale.test.ts`, "Boss áp CÙNG quy tắc ×2"). This is a deliberate behavior CHANGE, not an addition: boss must render at `2 × ENEMY_DISPLAY_SCALE_MULTIPLIER` (4× the base PNG), while regular enemies stay at the existing `ENEMY_DISPLAY_SCALE_MULTIPLIER` (2×) — the locked test's boss assertion must be updated to match, not left contradicting the new behavior.
