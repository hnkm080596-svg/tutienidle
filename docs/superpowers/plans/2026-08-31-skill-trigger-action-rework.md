...
- Existing saves keep any already-migrated skill (Huy Kiếm included) on its old `effects` data forever, since `GameManager.ts`'s template-refresh block doesn't touch `effects`/`triggers` — a save-shape decision the final whole-branch review raised (Important #4) and the user explicitly ruled on: no per-save migration/refresh logic, ever (matches the project's standing no-migration convention — see `game/docs/naming-conventions.md`'s save-version rule). When the full rework across all 6 paths ships, bump `CURRENT_SAVE_VERSION` then and let incompatible saves hit the existing block+Export/Backup screen — do not build a refresh path for `effects`/`triggers` before that.
