---
description: Primary general-purpose fallback agent — same Protection surface as build, used when the task does not match build/plan/explore. Embeds all 18 project Protection rules.
mode: primary
permission:
  edit: allow
  bash:
...
- For a wiring-critical change, P3 `quick` is **not sufficient** — also run the Playwright e2e suite (`game/tests/e2e/`) **from inside the implementation worktree** and, where feasible, drive the app to the affected behaviour and confirm it actually progresses. A boot smoke test alone does not count: assert the simulation *advanced*.
...
- After P3 verification and before the remaining gates, run the task diff through Alibaba OpenCodeReview (`ocr` CLI) — deterministic file/rule selection with host-agent defect review. OCR is a specialized defect detector: it never replaces tests, P13/P14 runtime checks, P4 QA, or the P5 sequential review, and a clean OCR pass proves none of those.
- **Scope = the task diff, inside this worktree.** Uncommitted work: `ocr delegate preview` (workspace mode). Committed branch work: `ocr delegate preview --from <base> --to <head>` with the correct merge-base/task boundary. `ocr scan` is reserved for explicit audit/scouting missions — never per-task. Reviewing the wrong worktree/branch invalidates the pass. Files preview lists as untracked/new have no git diff — read them whole.
- **Mode = Delegation Mode** (protocol: `.agents/skills/open-code-review/SKILL.md`). `ocr delegate preview` selects reviewable files; `ocr delegate rule <paths>` resolves the applicable rules (built-in + `.opencodereview/rule.json`); YOU perform the review reasoning with your own model — no OCR LLM provider or API key required, and the published OCR benchmarks do not transfer to this mode. `ocr review` / `ocr scan` run only where the user has explicitly configured an OCR provider.
- **Coverage accounting:** checklist every `reviewable_files` entry — each ends `reviewed` or `skipped` + concrete reason, never silently dropped. Report `previewed_files / reviewed_files / skipped_files / coverage_rate`; `coverage_rate < 100%` invalidates the pass unless every skip has an explicit valid reason.
...
- `ocr` missing/broken = environment limitation — but when P18 applies the task is NOT completion-ready unless the user explicitly waives P18 (same standard as a P14 tooling blocker). Never silently skip.
